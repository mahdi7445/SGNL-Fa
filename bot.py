...
# 🔴 رفعِ همون کلاسِ باگِ Run #1522 (توضیح کامل بالای DAILY_REPORT_HOUR_UTC در همین فایل):
# os.environ.get(key, "1m") فقط وقتی متغیر کلاً غایب باشه پیش‌فرض "1m" رو می‌داد - نه وقتی
# secret تنظیم شده ولی خالیه (که از وقتی signals.yml این متغیر رو بدون قید و شرط پاس می‌ده،
# دقیقاً همین حالته اگه کاربر این secret رو ست نکرده باشه). چون خودِ کرش نمی‌کرد (فقط
# "".split(",") -> [""] -> فیلتر می‌شد -> set خالی)، این یکی بی‌صدا رفتار پیش‌فرضِ مستندشده
# (غیرفعال‌بودن ۱ دقیقه‌ای به‌صورت پیش‌فرض) رو نقض می‌کرد، نه کرش می‌کرد - خطرناک‌تر چون کسی
# متوجهش نمی‌شد.
