...
# 🔴 رفعِ ریسکِ drift - ولی با یک اصلاح مهم نسبت به نسخه‌ی قبلی: طبق تأییدِ صریح کاربر،
# bot.py و shared_risk_config.py در دو ریپوی گیت کاملاً جداگانه‌اند (ریپوی SGNL-Fa فقط
# bot.py دارد، shared_risk_config.py اصلاً در آن وجود ندارد). یعنی `from shared_risk_config
# import ...` به‌شکلِ قطعی/اجباری، همین الان با ModuleNotFoundError در همون خط اول اجرای
# ربات کرش می‌کرد - نسخه‌ی قبلی این فایل این مشکل را نداشت و اصلاً تست نشده بود روی این
# ساختارِ دو-ریپو.
#
# راه‌حل: import داخل try/except با یک fallback محلی که *دقیقاً* همون مقادیرِ فعلیِ
# shared_risk_config.py را دارد (پس امروز هیچ رفتاری فرق نمی‌کند و هیچ باگی برنمی‌گردد) -
# ولی اگر import موفق شد (یعنی بعداً یک قدم CI اضافه شد که shared_risk_config.py را از
# ریپوی دیگر می‌کشد - نگاه کنید توضیح جداگانه‌ای که برای workflow دادم)، assertهای
# ضدِ drift پایین‌تر خودکار فعال می‌شوند. اگر import ناموفق بود (وضعیت فعلی)، این ربات
# دیگر کرش نمی‌کند و با مقادیر محلیِ درست (هم‌ارز با shared_risk_config.py) کار می‌کند -
# فقط دیگر گارد خودکارِ drift ندارد تا وقتی آن قدم CI اضافه شود.
try:
    from shared_risk_config import (
        W1 as _CANON_W1, W2 as _CANON_W2, W3 as _CANON_W3, W4 as _CANON_W4,
        W_RUNNER as _CANON_W_RUNNER, RR_TARGETS as _CANON_RR_TARGETS,
        TRAILING_R_MULT as _CANON_TRAILING_R_MULT, VALID_TIMEFRAME_LABELS,
        TARGET_ACTION_LINE, format_signal_price as _shared_format_signal_price,
    )
    _SHARED_RISK_CONFIG_AVAILABLE = True
except ModuleNotFoundError:
    _SHARED_RISK_CONFIG_AVAILABLE = False
    # مقادیرِ fallback - کپیِ دقیقِ همون چیزی که همین الان در shared_risk_config.py
    # (ریپوی TICB-En) هست. اگر روزی shared_risk_config.py مقادیرش عوض شد، این‌ها هم
    # باید دستی به‌روز بشن (تا وقتی fetch خودکار بین دو ریپو راه‌اندازی بشه).
    _CANON_W1, _CANON_W2, _CANON_W3, _CANON_W4, _CANON_W_RUNNER = 0.20, 0.30, 0.15, 0.10, 0.25
    _CANON_RR_TARGETS = [1, 2, 4, 6]
    _CANON_TRAILING_R_MULT = 1.5
    ...
    def _shared_format_signal_price(value: float) -> str:
        ...
        if av >= 1:
            return f"{v:,.2f}"
        decimals = min(12, max(2, -int(_math.floor(_math.log10(av))) + 3))
        return f"{v:.{decimals}f}"
# توجه: compute_final_r عمداً از shared_risk_config import نمی‌شود - نسخه‌ی محلیِ این فایل
# (پایین‌تر) دقیقاً همان چیزی است که shared_risk_config.compute_final_r هم بعد از رفعِ
# باگِ رانر به‌سویش اصلاح شد (جمعِ واقعیِ frac×r از trade["fills"]، نه یک فرمولِ تقریبی) -
# پس اینجا از قبل چیزِ درست‌تری موجود بود؛ چیزی برای جایگزینی نیست.
...
CRYPTOMETER_API_URL = (os.getenv('CRYPTOMETER_API_URL') or 'https://cryptometer.io').strip()
...
def format_signal_price(price: float) -> str:
    """اعداد سیگنال بدون علامت $ - الان نام مستعارِ محلی برای shared_risk_config.
    format_signal_price است. رفعِ ناهماهنگیِ ظاهری با candle_engine.py: قبلاً این تابع
    نه جداکننده‌ی هزارگان داشت (یک قیمت مثل 67543.21 اینجا بدون کاما، ولی در ربات VIP با
    کاما «67,543.21» نشون داده می‌شد - و چون اسکن TOP30 همین ربات می‌تونه کوین‌های
    گران‌قیمت مثل BTC/ETH/BNB را هم پوشش بده، این ناهماهنگی واقعاً در همون کانال دیده
    می‌شد)، و نه دقیقاً همون قاعده‌ی رقم اعشارِ پویا (بر اساس log10 قیمت) را برای
    قیمت‌های زیر ۱ دلار داشت (آستانه‌های ثابتِ قبلی گاهی رقم اعشار کمتری از حدِ لازم
    برای ۳ رقم معنادار می‌دادند). نام تابع عمداً همون قبلی نگه داشته شده تا نیازی به
    تغییر محل‌های فراخوانی در این فایل نباشه."""
    return _shared_format_signal_price(price)
...
def save_state(state: Dict[str, Any]):
    ...
    try:
        ...
        with open(tmp_path, 'w', encoding='utf-8') as f:
            ...
            # 🔴 رفعِ خلأِ دوامِ داده (نسبت به shared_git_sync.atomic_write_json که این دو خط
            # را از اول داشت): بدون flush+fsync، os.replace زیر فقط تضمین می‌کند که rename در
            # سطح فایل‌سیستم اتمیک است - نه اینکه محتوای فایل موقت واقعاً به دیسک رسیده باشد
            # (ممکن است هنوز فقط در بافر نوشتاریِ سیستم‌عامل باشد). با یک قطعیِ ناگهانیِ برق/
            # کرش سخت‌افزاری دقیقاً همون لحظه، فایلِ جایگزین‌شده می‌توانست خالی/ناقص باشد.
            f.flush()
            os.fsync(f.fileno())
...
def _load_small_json_file(path: str, default: Any) -> Any:
    """برای فایل‌های کوچکِ کمکیِ جداگانه از state.json (signal_id_counter.json،
    daily_report_log.json) - همان الگوی atomic که save_state دارد، فقط تعمیم‌یافته
    برای هر مسیر دلخواه."""
    if os.path.exists(path):
        try:
            ...
            logger.warning(f"⚠️ خطا در خواندن {os.path.basename(path)}، شروع با مقدار پیش‌فرض: {e}")
    return default


def _save_small_json_file(path: str, data: Any) -> None:
    try:
        tmp_path = path + ".tmp"
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
            # همان رفعِ خلأِ دوامِ داده‌ی save_state - نگاه کنید کامنتِ آن تابع.
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp_path, path)
    except Exception as e:
        logger.error(f"❌ خطا در ذخیره‌ی {os.path.basename(path)}: {e}")


# ================== شناسه‌ی یکتای هر سیگنال ==================
#
# طبق درخواست صریح کاربر - این بات (نسخه‌ی آلت‌کوینِ کانال VIP) هم باید دقیقاً همان
# سیستم رهگیریِ سیگنال (شناسه‌ی یکتا + مسیر حرکتیِ کامل) را داشته باشد که برای
# candle_engine.py/subscription_bot.py (ربات VIP اصلی، بیت‌کوین/اتریوم) ساخته شد.
#
# 🔴 نکته‌ی حیاتی که کاربر صریحاً تاکید کرد: چون سیگنال‌های این بات به کانال VIP هم
# کپی می‌شوند (از طریق مکانیزم رله‌ی خودکار subscription_bot.py)، شناسه‌ی این بات
# هرگز نباید با شناسه‌ی ربات VIP («TC-000001»، «TC-000002»، ...) تداخل داشته باشد -
# چون هر دو در نهایت کنار هم در همان یک کانال دیده می‌شوند. برای همین، پیشوند این
# بات کاملاً متفاوت و غیرقابل‌اشتباه‌گرفتن انتخاب شد: «ALT-» (نه «TC-») - صرف‌نظر از
# مقدار خودِ شمارنده، دو پیشوند متفاوت تضمین می‌کند که هیچ‌وقت هیچ شناسه‌ای از این دو
# سیستم برابر نشود.
#
# 🔴 چرا یک فایل کاملاً جداگانه (نه یک فیلد داخل state.json): همان دلیلی که باعث شد
# سیستم «گزارش روزانه»ی قدیمی این فایل حذف بشه (توضیح کامل بالای maybe_send_daily_report
# پایین‌تر) - اگر این شمارنده داخل state.json (همان دیکشنری‌ای که در main() با
# copy.deepcopy پشتیبان گرفته و در صورت خطای run_scan_cycle به آن نسخه‌ی قدیمی‌تر
# revert می‌شود) بود، یک سناریوی نادر ولی واقعی وجود داشت: یک سیگنال با شناسه‌ی
# «ALT-000042» باز و پیامش با موفقیت به کانال ارسال می‌شد، ولی بعداً در همان دور
# اسکن (روی نماد/تایم‌فریم دیگری) یک خطای غیرمنتظره رخ می‌داد و کل state (شامل
# شمارنده‌ی تازه‌افزایش‌یافته) به نسخه‌ی قبل از این دور برمی‌گشت - یعنی سیگنال بعدی
# دوباره «ALT-000042» می‌گرفت، برای یک معامله‌ی کاملاً متفاوت که پیامش از قبل با
# همین شناسه در کانال منتشر شده. با نگه‌داشتن این شمارنده در یک فایل کاملاً جدا
# (خارج از state و خارج از چرخه‌ی backup/revert آن)، این ریسک کاملاً حذف می‌شود -
# دقیقاً همان الگویی که candle_engine.py (ربات VIP) هم استفاده می‌کند.
SIGNAL_ID_COUNTER_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "signal_id_counter.json")
# 🔴 هماهنگ‌سازی با ربات مرجع (candle_engine.py، ربات VIP): همون‌جا SIGNAL_ID_PREFIX به یک
# متغیر محیطی تبدیل شد (پیش‌فرض «TC») تا هیچ مقدار حیاتی داخل کد هاردکد نمونه، صرفاً برای
# انعطاف عملیاتی (مثلاً تست با یک پیشوند متفاوت بدون تغییر کد). این‌جا هم دقیقاً همون الگو
# اعمال شد - پیش‌فرض همچنان «ALT» می‌مونه (بدون این env، هیچ رفتاری عوض نمی‌شه)، فقط دیگه
# قفل‌شده در کد نیست.
#
# ⚠️ توجه مهم: این بات (bot.py) و ربات VIP (candle_engine.py) دو کدبیسِ کاملاً مستقل و
# جداگانه‌اند (نه یک فایل مشترک با env متفاوت) - پس این تغییر صرفاً برای یکدست‌شدنِ الگو با
# ربات مرجع است، نه یک وابستگیِ واقعیِ کد. تا وقتی این env ست نشه (که پیش‌فرض هم همینه)،
# پیشوند «ALT» دقیقاً مثل قبل بدون تغییر باقی می‌مونه - و همچنان باید همیشه با پیشوند ربات
# VIP («TC») متفاوت بمونه (چون سیگنال‌های این بات به همون یک کانال کپی می‌شن)، هرگز این env
# رو برابر TC نذارید.
SIGNAL_ID_PREFIX = (os.environ.get("SIGNAL_ID_PREFIX", "ALT").strip().upper() or "ALT")


def _signal_id_in_use(signal_id: str, global_state: Optional[Dict[str, Any]]) -> bool:
    """چک می‌کند یک signal_id همین الان روی یک معامله‌ی هنوز-باز یا یک معامله‌ی از قبل
    بسته‌شده استفاده شده یا نه - عیناً همان الگوی ربات VIP (candle_engine.py)، با این تفاوت
    که چون این بات trade_history را همیشه در همان global_state درون-حافظه‌ای نگه می‌دارد
    (نه یک فایل جدا که باید هر بار خوانده شود)، این چک هیچ I/O اضافه‌ای نمی‌طلبد."""
    if global_state is None:
        return False
    for s in global_state.get("candle_signals", {}).values():
        t = (s or {}).get("open_trade")
        if t and not t.get("closed") and t.get("signal_id") == signal_id:
            return True
    for h in global_state.get("trade_history", []):
        if h.get("signal_id") == signal_id:
            return True
    return False


def next_signal_id(global_state: Optional[Dict[str, Any]] = None) -> str:
    """🔴 باگ رفع‌شده: قبلاً این تابع هیچ گاردی نداشت - صرفاً به مقدار فایلِ
    signal_id_counter.json اعتماد می‌کرد و همون رو بدون هیچ چکی برمی‌گردوند. اگه این فایل
    به هر دلیلی (دو نمونه‌ی هم‌پوشانِ این اسکریپت، یا یک بازگردانیِ گیت به یک نسخه‌ی قدیمی‌تر
    بعد از یک تعارضِ پایدار) به یک مقدارِ عقب‌تر برمی‌گشت، یک شناسه‌ی از قبل استفاده‌شده
    (چه روی یک معامله‌ی هنوز-باز، چه یک معامله‌ی از قبل بسته‌شده) بدون هیچ هشداری دوباره
    صادر می‌شد - دقیقاً همون کلاس‌باگی که در ربات VIP (candle_engine.py) پیدا و رفع شد. الان
    - عیناً همون الگو - قبل از برگردوندنِ شناسه، در برابر هم معاملاتِ هنوز-باز و هم
    trade_history چک می‌شه؛ اگه تصادم پیدا شد، تا رسیدن به یک شناسه‌ی واقعاً آزاد جلو می‌ره و
    یک هشدار به ادمین می‌فرسته."""
    current = _load_small_json_file(SIGNAL_ID_COUNTER_FILE, {"next": 1})
    n = int(current.get("next", 1))
    skipped = 0
    candidate = f"{SIGNAL_ID_PREFIX}-{n:06d}"
    if global_state is not None:
        while _signal_id_in_use(candidate, global_state):
            n += 1
            skipped += 1
            candidate = f"{SIGNAL_ID_PREFIX}-{n:06d}"
        if skipped:
            logger.warning(f"⚠️ next_signal_id: {skipped} شناسه‌ی از قبل در حال استفاده رد شد - "
                            f"از {candidate} ادامه می‌دهیم.")
            notify_admin(
                f"{skipped} colliding signal_id(s) skipped before minting {candidate} - this almost "
                f"certainly means either two overlapping instances of bot.py running at once, or that "
                f"signal_id_counter.json was reverted to an older value (e.g. by a git conflict/reset).",
                global_state, kind="signal_id_collision")
    _save_small_json_file(SIGNAL_ID_COUNTER_FILE, {"next": n + 1})
    return candidate


def _new_trade_event(event_type: str, price: Optional[float] = None, r: Optional[float] = None,
                      note: Optional[str] = None) -> Dict[str, Any]:
    """یک رکورد استانداردِ «مسیر حرکتی» با timestamp دقیق - عیناً همان الگوی
    candle_engine.py (ربات VIP) - تا کل تاریخچه‌ی یک سیگنال (باز شدن -> هر تارگت/
    استاپ با زمان و قیمت دقیق -> بسته‌شدن نهایی) حتی سال‌ها بعد هم قابل بازسازی باشد."""
...
# 🔴 رفعِ باگِ «هیچ‌کدام از subprocess های گیت اینجا timeout ندارند»: shared_git_sync.py
# (که candle_engine.py/subscription_bot.py استفاده می‌کنن) دقیقاً همین مشکل رو به‌عنوان
# یکی از جدی‌ترین دلایل قطعیِ کامل مستند کرده: اگه یک `git pull`/`push` روی یک اتصال
# شبکه‌ی نیمه‌قطع (stall، نه reject سریع) گیر کنه، بدون timeout کل پروسه‌ی پایتون برای
# همیشه (یا تا سقف سخت‌گیرانه‌ی خودِ GitHub Actions) بی‌حرکت می‌مونه - نه کرش می‌کنه که
# except بیرونی بگیردش، نه ادامه می‌ده. چون این فایل (bot.py) از shared_git_sync.py
# استفاده نمی‌کنه (پیاده‌سازیِ گیتِ کاملاً مستقلِ خودش رو داره - نگاه کنید docstring پایین)،
# همین آسیب‌پذیریِ دقیقاً مستندشده تا این اصلاح اینجا هم وجود داشت. مقدار (۴۵ ثانیه) عمداً
# همون عددیه که shared_git_sync.GIT_SUBPROCESS_TIMEOUT_SECONDS دارد.
...
def git_commit_and_push():
    ...
    try:
        ...
        # 🔴 اضافه شد: signal_id_counter.json و daily_report_log.json - دو فایل کاملاً
        # جداگانه (خارج از state.json و خارج از چرخه‌ی backup/revert آن در main()) که
        # برای سیستم شناسه‌ی سیگنال و گزارش روزانه اضافه شدند (توضیح کامل بالای
        # next_signal_id و maybe_send_daily_report). اگر این‌ها اضافه نمی‌شدند، هیچ‌وقت
        # commit/push نمی‌شدند - یعنی هر اجرای جدید (checkout تازه از ریموت) دوباره از
        # شمارنده‌ی صفر/پرچمِ خالی شروع می‌کرد: شناسه‌های تکراری می‌ساخت و گزارش روزانه
        # را هر روز دوباره می‌فرستاد - دقیقاً همان دو باگی که قرار بود این معماری (فایل
        # جدا) حذفشان کند.
        # ⚠️ نکته‌ی مهم: این دو فایل فقط وقتی اولین بار next_signal_id/
        # maybe_send_daily_report صدا زده بشن ساخته می‌شن - یعنی در همون اجرای اول
        # (checkout تازه، قبل از هر معامله/گزارشی) ممکنه هنوز روی دیسک نباشن.
        # `git add` با یک pathspec که هیچ فایلی را match نکنه، حتی با چند فایل دیگر
        # هم‌زمان، کل دستور را با خطا (fatal: pathspec did not match) متوقف می‌کند -
        # یعنی اگر این دو اسم همیشه بی‌قید-و-شرط به یک `git add` واحد داده می‌شدند،
        # قبل از این‌که این فایل‌ها اولین‌بار ساخته بشن، حتی خودِ state.json هم دیگر
        # commit نمی‌شد (چون کل این دستور fatal می‌شد و try/except بیرونی همه‌چیز،
        # شامل state.json را هم، رد می‌کرد). به همین دلیل فقط فایل‌هایی که واقعاً روی
        # دیسک موجودند به لیست اضافه می‌شوند.
        files_to_add = ["state.json"]
        for extra in ("signal_id_counter.json", "daily_report_log.json"):
            if os.path.exists(os.path.join(repo_dir, extra)):
        ...
        logger.warning(f"⚠️ [git] یک فراخوانیِ گیت بعد از {GIT_SUBPROCESS_TIMEOUT_SECONDS} ثانیه هنگ کرد و "
                        f"متوقف شد (احتمالاً اتصال شبکه‌ی نیمه‌قطع) - دور بعدی این حلقه دوباره تلاش می‌کند: {e}")
    except Exception as e:
        logger.warning(f"⚠️ [git] کامیت/پوش ناموفق بود: {e}")


# 🔴 رفعِ باگِ «پیام‌های سیگنال‌های قدیمی/بسته‌شده دوباره ارسال می‌شوند»: ریشه‌ی واقعی
# این بود که رصد وضعیت هر معامله (تارگت خورد / استاپ خورد / بسته شد) فقط در حافظه
# فوری بود، ولی ذخیره‌ی دائمی (save_state + git push، تنها منبعی که یک اجرای تازه
# بعد از crash/timeout/kill از رویش state را می‌خواند) فقط دوره‌ای بود: هر ۲ دقیقه
# (GIT_COMMIT_EVERY_SECONDS) یا انتهای هر دور اسکن کامل (~۵ دقیقه یا بیشتر). اگر
# پروسه دقیقاً در همین فاصله (بین یک پیام واقعاً ارسال‌شده + پرچمِ trade["hit"]/
# ["closed"] ست‌شده در حافظه، و اولین ذخیره‌ی دوره‌ای بعدی) متوقف می‌شد - چه به‌خاطر
# timeout ۵ ساعت و ۲۰ دقیقه، چه crash، چه توقف runner - آن پرچم هرگز به ریموت
# نمی‌رسید. اجرای بعدی state قدیمی‌تر (بدون آن پرچم) را از ریموت می‌خواند، همان
# کندل‌های تاریخی را دوباره از Twelve Data می‌گیرد (که هیچ‌وقت عوض نمی‌شوند)، دوباره
# به همان تارگت/استاپِ از قبل رخ‌داده و پیام‌شده می‌رسد، و دوباره پیام "Target N HIT"
# یا بسته‌شدن را می‌فرستد - برای معامله‌ای که واقعاً هفته‌ها/روزها پیش بسته شده.
#
# راه‌حل: به‌جای اتکا فقط به ذخیره‌ی دوره‌ای، همین که یک پیام واقعاً به کانال تلگرام
# تحویل داده شد (نه فقط تلاش برای ارسال - موفقیت واقعی)، بلافاصله همان‌جا state
# ذخیره و پوش می‌شود - قبل از رفتن سراغ نماد/رویداد بعدی. یعنی حداکثر چیزی که با یک
# kill ناگهانی از دست می‌رود، دقیقاً همان یک پیامِ در حال ارسال است (که چون هنوز
# موفق نشده، اصلاً trade را closed/hit علامت نزده) - نه انباشتی از چند دقیقه رویداد
# قبلی که همه‌شان واقعاً به کانال رسیده بودند. چون این تابع فقط وقتی صدا زده می‌شود
# که واقعاً پیامی ارسال شده (نه هر چند ثانیه در حلقه‌ی اصلی)، فرکانس push هم به
# فرکانس واقعی سیگنال‌ها محدود می‌ماند - نه هر تیک WebSocket - پس هیچ فشار اضافه‌ای
# روی حلقه‌ی اصلی/API گیت‌هاب نمی‌گذارد.
def _persist_immediately(state: Optional[Dict[str, Any]], reason: str = "") -> None:
    if state is None:
        return
    try:
        save_state(state)
        ...
        # هرگز نباید این تابع خودش باعث قطع مسیر ارسال سیگنال شود - شکست اینجا فقط
        # لاگ می‌شود؛ ذخیره‌ی دوره‌ای معمولی (هر ۲ دقیقه) همچنان به‌عنوان لایه‌ی دوم
        # پشتیبان کار می‌کند.
        logger.warning(f"⚠️ ذخیره/پوش فوری بعد از ارسال پیام ({reason}) ناموفق بود - "
                        f"لایه‌ی دوم (ذخیره‌ی دوره‌ای هر ۲ دقیقه) همچنان پشتیبان است: {e}")
...
# 🔴 گارد ضدِ drift (رفعِ همون کلاس‌باگی که shared_risk_config.py برایش ساخته شد، اما این
# فایل تا الان بیرونش مونده بود): چون این فایل به دلایل ساختاریِ خودش (منطق باز/بسته‌شدنِ
# معامله در check_open_trade به فرمتِ list-of-dict نیاز داره) نمی‌تونه مستقیماً همون
# ثابت‌های canonical رو جایگزین TARGET_LEVELS محلی‌اش کنه، حداقل هر بار که این ربات بالا
# می‌آد، مقادیرِ محلی رو در برابر منبعِ حقیقتِ مشترک (shared_risk_config.py) چک می‌کنه. اگه
# یک‌نفر فقط shared_risk_config.py (یا فقط همینجا) رو ویرایش کنه بدون هماهنگ‌کردنِ دیگری،
# ربات به‌جای سکوت/drift خاموش، همون لحظه با AssertionError واضح متوقف می‌شه.
assert RR_TARGETS == _CANON_RR_TARGETS, (
    f"bot.py RR_TARGETS {RR_TARGETS} drifted from shared_risk_config.RR_TARGETS "
    f"{_CANON_RR_TARGETS} — update both together."
)
assert [lvl["close_frac"] for lvl in TARGET_LEVELS] == [_CANON_W1, _CANON_W2, _CANON_W3, _CANON_W4], (
    "bot.py TARGET_LEVELS close_frac values drifted from shared_risk_config.py's W1..W4 — "
    "update both together."
)
assert abs(RUNNER_FRACTION - _CANON_W_RUNNER) < 1e-9, (
    f"bot.py RUNNER_FRACTION ({RUNNER_FRACTION}) drifted from shared_risk_config.W_RUNNER "
    f"({_CANON_W_RUNNER}) — update both together."
)
assert TRAILING_DISTANCE_R == _CANON_TRAILING_R_MULT, (
    f"bot.py TRAILING_DISTANCE_R ({TRAILING_DISTANCE_R}) drifted from "
    f"shared_risk_config.TRAILING_R_MULT ({_CANON_TRAILING_R_MULT}) — update both together."
)
...
TIMEFRAMES = {
    ...
    # 🔴 اضافه شد: طبق درخواست صریح کاربر - این تایم‌فریم قبلاً فقط در ربات VIP
    # (candle_engine.py، روی BTC/ETH) بود؛ چون طبق بک‌تستِ کاربر ۴ ساعته بیشترین سودِهی
    # رو بین همه‌ی تایم‌فریم‌ها داشته، الان دقیقاً همون منطق (fetch/تشخیص سیگنال/۴
    # تارگت/تریلینگ/رانر - چیزی که TIMEFRAMES از قبل برای بقیه فراهم می‌کنه) روی واچ‌لیست
    # اصلی (SOL/XRP/ADA) هم فعال شد. catchup_limit=10 یعنی هر بار تا ۱۰ کندلِ ۴ساعته
    # (۴۰ ساعت) عقب‌ماندگی رو می‌تونه یک‌جا جبران کنه - بیشتر از کافی برای فاصله‌ی
    # بین دو اجرای self-chained (~۵ ساعت و ۲۰ دقیقه).
    #
    # ⚠️ نکته‌ی بودجه: این ۳ نماد × ۶ بار/روز = ۱۸ درخواست/روز به سهمیه‌ی روزانه‌ی
    # Twelve Data اضافه می‌شه. چون مجموعِ قبلی (~۸۷۸ در بدترین حالتِ نظری) از قبل به
    # سقفِ رایگان ۸۰۰/روز نزدیک بود، هم‌زمان با این تغییر، TOP30_SCAN_HOURS (پایین‌تر)
    # از ۳ بار/روز به ۲ بار/روز کم شد تا ۳۰ درخواست/روز آزاد بشه - یعنی نه‌فقط جای این
    # ۱۸ تا باز شد، ۱۲ تا هم حاشیه‌ی امنِ اضافه موند. این انتخاب (کم‌کردنِ TOP30، نه
    # واچ‌لیستِ اصلی) عمداً بود: TOP30 فقط یک اسکنِ گسترده‌ی تک‌تایم‌فریمی (۱ساعته) روی
    # ۳۰ ارزه، نه یک ردیابیِ کامل مثل واچ‌لیستِ اصلی - کمترین اثر رو روی کیفیتِ سیگنال‌ها
    # داره؛ فقط از هر ۸ ساعت به هر ۱۲ ساعت چک می‌شه.
    "4h":  {"td_interval": "4h",    "bar_seconds": 4 * 60 * 60,      "label": "4H",  "catchup_limit": 10},
...
# 🔴 گارد ضدِ drift - دقیقاً هم‌الگو با assert مشابه در candle_engine.py: subscription_bot.py
# سیگنال‌های دستی/فوروارد‌شده را در برابر VALID_TIMEFRAME_LABELS (در shared_risk_config.py)
# اعتبارسنجی می‌کند؛ اگه اینجا یک تایم‌فریمِ جدید اضافه/حذف بشه بدون اینکه اون لیستِ مشترک
# هم به‌روز بشه، این assert همون لحظه‌ی بالا اومدنِ ربات خطای واضح می‌ده - نه سکوت.
assert {cfg["label"] for cfg in TIMEFRAMES.values()} == VALID_TIMEFRAME_LABELS, (
    "bot.py TIMEFRAMES labels drifted from shared_risk_config.VALID_TIMEFRAME_LABELS — "
    "update both together."
)

# 🔴 اضافه شد: طبق بک‌تستِ کاربر («تایم‌فریم ۱ دقیقه از اول سوددهی نداشت»، تأیید‌شده هم با
# دیتای بک‌تست هم با ۵۰ معامله‌ی زنده‌ی اخیر: 1m=۳۰ معامله، ۴۷٪ برد، -۵.۴R در برابر
# 5m/15m/1h که هرکدوم مثبت بودن) - یک راه کاملاً برگشت‌پذیر و بدون تغییر در بقیه‌ی منطق
# برای خاموش/روشن‌کردنِ صدور سیگنالِ خودکار روی یک تایم‌فریمِ خاص، بدون هیچ تغییر کدی. فقط
# جلوی *باز شدنِ معامله‌ی جدید* رو می‌گیره (در process_and_send_symbol_tf::_try_open_trade
# پایین‌تر) - معاملاتی که از قبل روی این تایم‌فریم باز بودن (اگه قبل از غیرفعال‌شدن باز
# شده باشن) دقیقاً مثل همیشه رهگیری/بسته می‌شن، هیچ اثرِ جانبی‌ای روی بقیه‌ی سیستم نداره.
# پیش‌فرض «1m» است (طبق تصمیم صریح کاربر). برای تغییرش از طریق Secret این ریپو
# (DISABLED_AUTO_TIMEFRAMES) - و **فقط** از همین طریق، چون این متغیر در کل کد فقط همین یک‌بار
# در پایین از env خونده می‌شه و بعدش هیچ‌جای دیگه دوباره نوشته نمی‌شه:
#   - Secret را حذف کنید یا کاملاً خالی بذارید  → پیش‌فرض («1m» غیرفعال) برقرار می‌مونه.
#   - Secret را دقیقاً به "none" ست کنید          → هیچ تایم‌فریمی غیرفعال نمی‌شه (۱ دقیقه هم فعال).
#   - Secret را به لیستی مثل "1m,5m" ست کنید      → دقیقاً همون‌ها غیرفعال می‌شن.
# 🔴 رفعِ باگِ واقعی (همون کلاسِ باگِ candle_engine.py، توضیح کامل آنجا): قبلاً «خالی بذارید»
# (چه با حذفِ کامل Secret چه با ست‌کردنش به مقدار خالی) هر دو به یک نتیجه می‌رسیدن (پیش‌فرضِ
# "1m") - یعنی هیچ راهی برای گفتنِ صریحِ «هیچ‌کدام رو غیرفعال نکن» نبود. حالا با یک مقدارِ
# صریح ("none") این ابهام کاملاً برطرف شده.
_DISABLED_TF_RAW = (os.environ.get("DISABLED_AUTO_TIMEFRAMES") or "").strip().lower()
if _DISABLED_TF_RAW == "none":
    DISABLED_AUTO_TIMEFRAMES = set()
else:
    DISABLED_AUTO_TIMEFRAMES = set(
        tf.strip().lower() for tf in (_DISABLED_TF_RAW or "1m").split(",") if tf.strip()
    )
...
def _tf_window_id(tf_key: str, now_utc: datetime) -> Optional[str]:
    ...
    if tf_key == "4h":
        # 🔴 اضافه شد: بازه‌بندیِ ۴ساعته‌ی روز (۰۰-۰۴, ۰۴-۰۸, ...، دقیقاً هم‌راستا با
        # نحوه‌ی بستن کندل‌های ۴ساعته‌ی واقعیِ بازار از نیمه‌شب UTC) - هر پنجره فقط یک‌بار
        # پردازش می‌شه، مثل بقیه‌ی تایم‌فریم‌ها.
...
#   4h  هر ۴ ساعت   (۶ بار/روز)  × ۳ = ۱۸  (اضافه شد - نگاه کنید تعریف TIMEFRAMES بالاتر)
#   جمع واچ‌لیست اصلی = ۶۶۶ درخواست/روز
# باقی‌مانده برای ۳۰ ارز برتر (۱ساعته، ۲بار/روز ≈ ۶۰/روز - قبلاً ۳بار/روز≈۹۰/روز بود؛
# طبق تصمیم صریح کاربر برای آزادکردنِ جا برای ۴ساعته کم شد، نگاه کنید TOP30_SCAN_HOURS
# پایین‌تر) + ارزهای پرتحرک (پایین‌تر ≈ ۶۶/روز) + رصد زنده‌ی REST معاملات باز
# غیر-واچ‌لیست (≈ ۵۰/روز، MAX_LIVE_CHECKS_PER_RUN) ≈ ۱۷۶ درخواست/روز - جمع کل ≈ ۸۴۲...
# در واقع همیشه کمتر از این سقفِ نظریِ بدترین‌حالت (چون نه همیشه معامله‌ی باز برای رصد
# هست، نه همیشه هر تایم‌فریمی سیگنال تازه دارد)؛ اگر عملاً هم به سقف نزدیک شد، اول
# TREND_ALTS_*_SCAN_HOURS را کاهش دهید، نه واچ‌لیست اصلی یا TOP30 را بیشتر از این.
#
...
# ⚠️ این کامنت طبق تصمیم قدیمی‌تر بود؛ کاربر بعداً صریحاً درخواست کرد تایم‌فریم ۴ ساعته هم
# (فقط برای واچ‌لیستِ اصلی SOL/XRP/ADA، نه این گروه) اضافه بشه - نگاه کنید تعریف TIMEFRAMES
# بالاتر. این گروه (TOP30) همچنان روی ۱ساعته می‌مونه، چون خودش یک اسکنِ گسترده (۳۰ نماد) با
# فرکانسِ عمداً پایینه، نه یک ردیابیِ دقیق مثل واچ‌لیستِ اصلی. دلیل محدودیتِ فرکانس: با
# سهمیه‌ی رایگان Twelve Data (۸۰۰ درخواست/روز) امکان چک‌کردن ۳۰ نماد با فرکانس معنادار
# وجود ندارد (هر دور کامل این گروه به‌تنهایی ۳۰ درخواست است) - به همین دلیل این گروه با
# فرکانس کم چک می‌شود تا مجموع سهمیه زیر ۸۰۰ درخواست در روز بماند و سهمیه‌ی بیشتری برای
# واچ‌لیست اصلی/ارزهای پرتحرک (که واقعاً به سرعت/دقت تایم‌فریم پایین نیاز دارند) آزاد بماند.
#
# 🔴 اضافه شد: از ۳ بار/روز به ۲ بار/روز (هر ۱۲ ساعت) کم شد - دقیقاً برای آزادکردنِ ۳۰
# درخواست/روز که صرفِ اضافه‌شدنِ تایم‌فریمِ ۴ ساعته به واچ‌لیستِ اصلی شد (۱۸ درخواست/روز
# لازم بود، ۱۲ تا هم حاشیه‌ی امن اضافه موند). این کمترین اثر رو روی کیفیتِ سیگنال‌ها داره،
# چون TOP30 خودش از قبل یک اسکنِ کم‌فرکانس/گسترده بود، نه یک ردیابیِ لحظه‌ای.
TOP30_ENABLED = True
TOP30_COUNT = 30
TOP30_TIMEFRAME = "1h"
TOP30_SCAN_HOURS = [0, 12]  # ۲ بار در روز (قبلاً ۳ بار بود - نگاه کنید کامنت بالا)
...
# 🔴 اضافه شد: طبق مشاهده‌ی صریح کاربر (تأیید‌شده هم با بک‌تست هم با ۵۰ معامله‌ی زنده‌ی
# اخیر: ارزهای کم‌حجم/میم‌کوین مثل FARTCOIN/USELESS/SKR سوددهیِ ضعیفی داشتن، در برابرِ
# ارزهای پرحجم که بهترین سود رو دادن) - دو آستانه‌ی قابل‌تنظیم (بدون تغییر کد، فقط env)
# برای فیلترکردنِ استخر کاندیدهای trend_alts، دقیقاً قبل از انتخاب/امتیازدهی (در
# get_trend_alts پایین‌تر). این‌ها روی WATCHLIST_SYMBOLS (SOL/XRP/ADA) اثر ندارن - آن‌ها
# از قبل با تصمیم دستی کاربر انتخاب شدن، نه از این استخرِ پویا.
#
# TREND_ALTS_MIN_VOLUME_USD: آستانه‌ی حجمِ ۲۴ساعته‌ی قبلی (۱۰ میلیون، هاردکد بود) الان
# قابل‌تغییر با env است - پیش‌فرض همون ۱۰ میلیونِ قبلی، تا رفتارِ فعلی بدون این env عوض
# نشه.
#
# TREND_ALTS_MIN_MARKET_CAP_USD: آستانه‌ی *جدید* - چون حجمِ ۲۴ساعته به‌تنهایی نمی‌تونه
# میم‌کوین‌ها رو filter کنه (این ارزها دقیقاً همون لحظه‌ی پامپ حجمِ بسیار بالایی می‌گیرن،
# حتی با market cap کوچیک - همون ریسکِ نوسانِ شدیدی که باعثِ نتیجه‌ی ضعیف می‌شه). پیش‌فرض
# ۵۰۰ میلیون دلار (رده‌ی mid-cap به بالا) - قابل تغییر یا کاملاً غیرفعال‌کردن (با مقدار 0)
# از طریق env، بدون نیاز به تغییر کد.
# 🔴 رفعِ باگ (همون کلاس‌باگِ DISABLED_AUTO_TIMEFRAMES بالاتر): بدون آرگومان دومِ پیش‌فرض،
# os.environ.get این‌ها None برمی‌گردوند و float(None) با TypeError کرش می‌کرد - یعنی این
# دو env، برخلاف چیزی که کامنت‌های بالا می‌گفتن («پیش‌فرض همون ۱۰ میلیون»/«پیش‌فرض ۵۰۰
# میلیون»)، در عمل اصلاً اختیاری نبودن؛ نبودشون کل ربات رو از کار می‌نداخت.
# 🔴 رفعِ همون باگ (کرشِ Run #1522، توضیح کامل بالای DAILY_REPORT_HOUR_UTC): این دو هم از
# قبل با os.environ.get(key, "...") (الگوی ناامن) نوشته شده بودن و چون از قبل هم در signals.yml
# بدون قید و شرط پاس داده می‌شدن، اگه این دو Secret تنظیم نشده باشن، دقیقاً همین کرش (رشته‌ی
# خالی -> float("") -> ValueError) رخ می‌داد - قبل از هر تغییری از من. با همون الگوی or درستش
# می‌کنیم.
TREND_ALTS_MIN_VOLUME_USD = float(os.environ.get("TREND_ALTS_MIN_VOLUME_USD") or "10000000")
TREND_ALTS_MIN_MARKET_CAP_USD = float(os.environ.get("TREND_ALTS_MIN_MARKET_CAP_USD") or "500000000")
...
def build_trend_alts_ws_map(state: Dict[str, Any]) -> Dict[str, str]:
    ...
    subscribe کردن استریم WebSocket + ساخت کندل ۱ دقیقه‌ای.

    🔴 رفعِ باگ: قبلاً فقط از get_active_trend_alts (لیست فعال/غیرقفل) استفاده می‌شد. مشکل:
    اگر نمادی به‌خاطر ۳ استاپِ پیاپیِ همان روز قفل شود ولی هنوز یک معامله‌ی باز روی تایم‌فریم
    ۱ دقیقه‌ای داشته باشد، آن نماد از نقشه‌ی WebSocket کاملاً حذف می‌شد - یعنی از اجرای بعدی
    دیگر هیچ تیکِ قیمتی برایش نمی‌رسید، کندل ۱ دقیقه‌ای‌اش دیگر هرگز ساخته نمی‌شد، و رصدِ
    معامله‌ی بازش (هم در check_trend_alts_1m_signals هم در check_ws_open_trades) عملاً متوقف
    می‌شد - برخلاف تضمین صریح بقیه‌ی کد که «معامله‌ی باز رها نمی‌شود» (نگاه کنید
    get_trend_alts_to_process). الان از همان تابع get_trend_alts_to_process استفاده می‌شود
    (فعال + هر قفل‌شده‌ای که هنوز معامله‌ی باز دارد) - این هیچ سیگنال ورود جدیدی برای نماد
    قفل‌شده صادر نمی‌کند (process_and_send_symbol_tf خودش مستقل is_symbol_blocked_today را
    قبل از باز کردن معامله‌ی جدید چک می‌کند)، فقط اجازه می‌دهد رصدِ معامله‌ی از قبل بازِ آن
    ادامه پیدا کند."""
    candle_states = state.get("candle_signals", {})
    result = {}
    for mover in get_trend_alts_to_process(state, candle_states):
        td_symbol = f"{mover['symbol']}/USD"
...
# 🔴 سیستم «گزارش روزانه‌ی خودکار» که این‌جا بود (RESULTS_REPORT_HOUR،
# is_results_report_due، mark_results_report_processed) طبق درخواست کاربر کاملاً
# حذف شد - چون با وجود چند لایه دفاعی پشت‌سرهم، همچنان به‌طور متناوب در ساعت‌های
# مختلف روز پیام تکراری می‌فرستاد (توضیح کامل بالای بلوکِ حذف‌شده در main، انتهای
# فایل). _refresh_tf_windows_from_disk پایین همچنان لازم است - چون این تابع الان
# عمومی است و برای سایر پنجره‌های tf_windows (flow_perf, top30, trendalts_*) هم
# استفاده می‌شود، نه فقط گزارش روزانه‌ی حذف‌شده.
...
def open_new_trade(signal: Dict[str, Any], global_state: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    entry = signal["price"]
    sl = signal["sl"]
    r = abs(entry - sl)
    if r <= 0:
        return None
    return {
        "signal_id": next_signal_id(global_state=global_state),
        ...
        # 🔴 تاریخچه‌ی کامل مسیر حرکتی این سیگنال - از همین لحظه تا بسته‌شدن نهایی
        # (log_trade_result این آرایه رو عیناً در trade_history ذخیره می‌کنه، پس بعد
        # از بسته‌شدن هم برای همیشه قابل بازیابیه).
...
def log_target_event(target_events: List[Dict[str, Any]], symbol: str, tf_key: str,
    ...
    زده می‌شد. هر رویداد با زمان واقعی وقوعش در state["target_events"] ذخیره
    می‌شود (لاگ خام، صرفاً برای بایگانی داخلی - سیستم گزارش روزانه‌ی خودکاری که
    قبلاً از این لاگ استفاده می‌کرد طبق درخواست کاربر کاملاً حذف شد؛ خودِ ثبت این
    رویدادها بی‌خطر است و دست‌نخورده ماند).
    """
...
def log_trade_result(trade_history: List[Dict[str, Any]], symbol: str, tf_key: str,
    ...
    # 🔴 رویدادِ نهاییِ «closed» - جدا از رویدادهای type-specific که قبلاً حین رسیدن به
    # هر تارگت/استاپ در trade["events"] ثبت شدن (check_open_trade/_force_close) - اینجا
    # (تنها نقطه‌ی مشترک برای همه‌ی مسیرهای بسته‌شدن) یک خلاصه‌ی نهایی با همون final_r
    # واقعی اضافه می‌شه، تا آخرین رکورد این تاریخچه همیشه دقیقاً نتیجه‌ی قطعی باشه.
    trade.setdefault("events", []).append(
        {"type": "closed", "ts": closed_at_iso, "close_type": close_type, "final_r": final_r})
    trade_history.append({
        # 🔴 اضافه شد: شناسه‌ی یکتای این سیگنال (پیشوند ALT- برای جلوگیریِ قطعی از
        # تداخل با شناسه‌های ربات VIP - توضیح کامل بالای next_signal_id) و کل مسیر
        # حرکتی (باز شدن -> هر رویداد با قیمت/زمان دقیق -> بسته‌شدن نهایی).
        ...
        # 🔴 اضافه شد: زمان واقعیِ صادرشدنِ سیگنال - جدا از closed_at (یک سیگنال می‌تواند
        # روزها باز بماند قبل از بسته‌شدن). برای گزارش روزانه («چند سیگنال امروز صادر شد»
        # جدا از «چند تا امروز بسته شد») لازم است - عیناً همان فیلدی که برای ربات VIP هم
        # اضافه شد.
...
# ================== گزارش روزانه‌ی خودکار ==================
#
# طبق درخواست صریح کاربر: «سیستم گزارش روزانه دقیقاً مانند کانال VIP هر روز در همان
# زمان فقط یک مرتبه، در این بات هم ایجاد و ارسال شود». سیستم قبلیِ این بات (بالاتر،
# کامنتِ حذف‌شده‌ی «🔴🔴 حذفِ کاملِ سیستم گزارش روزانه‌ی خودکار» را ببینید) دقیقاً به
# همین دلیل به‌طور کامل حذف شده بود: پرچمِ «امروز فرستاده شد» داخل همان state.json ای
# نگه داشته می‌شد که در main() با copy.deepcopy پشتیبان گرفته و در صورت خطای
# run_scan_cycle به نسخه‌ی قدیمی‌تر revert می‌شود - یعنی حتی اگر گزارش با موفقیت
# فرستاده و پرچمش mark+push هم شده بود، یک خطای بعدیِ کاملاً نامرتبط (روی نماد دیگری)
# در همان دور می‌توانست آن پرچم را از حافظه (و به‌زودی، با ذخیره‌ی دوره‌ای بعدی، از
# دیسک/ریموت هم) پاک کند - و اجرای بعدی دوباره due می‌دید و دوباره می‌فرستاد.
#
# راه‌حل این‌بار - دقیقاً همان الگویی که candle_engine.py (ربات VIP) از اول درست
# استفاده می‌کرد: پرچمِ «امروز فرستاده شد» در یک فایل کاملاً جداگانه (DAILY_REPORT_LOG_FILE)
# نگه داشته می‌شود - بیرون از state.json، بیرون از چرخه‌ی backup/revert آن. این فایل
# هرگز deepcopy/revert نمی‌شود چون اصلاً بخشی از دیکشنری state نیست؛ read/write آن
# کاملاً مستقل و مستقیم است. git_commit_and_push هم پایین‌تر طوری گسترش داده شده که
# این فایل (و signal_id_counter.json) را هم مثل state.json کامیت/پوش کند.
DAILY_REPORT_LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "daily_report_log.json")
DAILY_REPORT_HOUR_UTC = int(os.getenv("DAILY_REPORT_HOUR_UTC") or "21")  # همان پیش‌فرض ربات VIP - یعنی طبق پیش‌فرض دقیقاً همان ساعت
# 🔴 رفعِ باگِ واقعی (کرش‌کردنِ کل پروسه در Run #1522): os.getenv(key, "21") فقط وقتی متغیر
# محیطی *کلاً غایب* باشه به "21" برمی‌گرده - نه وقتی که موجود ولی خالی باشه. از وقتی
# candle_signals.yml/signals.yml این Secretها رو (حتی اگه کاربر تنظیمش نکرده باشه) بدون قید و
# شرط پاس می‌دن، GitHub Actions یک رشته‌ی خالیِ "" پاس می‌ده (نه غایب) - یعنی int("") و کرش
# فوری با ValueError. الگوی or (که بقیه‌ی این فایل و candle_engine.py/subscription_bot.py از
# اول همینو داشتن) هم رشته‌ی خالی هم غایب‌بودن رو یکسان به پیش‌فرض تبدیل می‌کنه.


def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]], issued_today: Optional[int] = None) -> str:
    """قالب و محتوا عیناً از format_daily_report_message در candle_engine.py (ربات VIP)
    کپی شد - طبق درخواست صریح («دقیقاً مانند کانال VIP»). فقط دو تفاوتِ ساختاریِ
    غیرقابل‌اجتناب با این بات تطبیق داده شد: (۱) targets_hit اینجا لیستی از اعداد
    ([1,2,3]) است نه برچسب رشته‌ای (["T1","T2"])، و (۲) تایم‌فریم واقعی است (tf همیشه
    یک مقدار معنادار مثل "5m"/"1h" دارد - نه "manual" مثل سیگنال‌های دستی/فوروارد‌شده‌ی
    ربات VIP)، پس نیازی به fallback روی logical_tf نیست."""
    ...
    for tf, hs in sorted(by_tf.items(), key=lambda kv: -sum(x["final_r"] for x in kv[1])):
        ...
        lines.append(f"  {tf_label}: {n} trade{'s' if n != 1 else ''} · {round(100 * w / n)}% win · {r:+.2f}R")

    # شمارشِ رسیدن به هر تارگت - targets_hit روی هر رکورد لیستی از اعداد است، مثلاً [1,2,3]
    target_reach = {lvl["n"]: 0 for lvl in TARGET_LEVELS}
    runner_closed = 0
    for h in trades:
        for n in h.get("targets_hit") or []:
            if n in target_reach:
                target_reach[n] += 1
        if h.get("close_type") == "runner_stop":
            runner_closed += 1
    lines.append("\n<b>Targets reached today</b>")
    for lvl in TARGET_LEVELS:
        lines.append(f"  T{lvl['n']} ({lvl['r']}R, {int(lvl['close_frac'] * 100)}%): "
                      f"{target_reach[lvl['n']]} reached")
    lines.append(f"  Runner (final {int(RUNNER_FRACTION * 100)}%, trailing {TRAILING_DISTANCE_R}R): "
                  f"{runner_closed} closed")

    return "\n".join(lines)


def _load_daily_report_log() -> List[str]:
    return _load_small_json_file(DAILY_REPORT_LOG_FILE, [])


def _save_daily_report_log(dates: List[str]) -> None:
    _save_small_json_file(DAILY_REPORT_LOG_FILE, dates[-60:])


def maybe_send_daily_report(bot: "TradeiscoolBot", state: Dict[str, Any]) -> None:
    """هر روز، یک‌بار بعد از ساعت DAILY_REPORT_HOUR_UTC، خلاصه‌ی عملکرد همه‌ی سیگنال‌های
    بسته‌شده‌ی آن روز را مستقیماً در کانال پست می‌کند - عیناً همان الگوی
    maybe_send_daily_report در candle_engine.py (ربات VIP): (۱) اول pull تازه (از طریق
    git_commit_and_push) تا اگر یک اجرای موازیِ دیگر همین الان گزارش را زده و پوش کرده،
    حتماً ببینیمش، (۲) mark-then-push-then-send: پرچم امروز بلافاصله ثبت+پوش می‌شود -
    قبل از اینکه اصلاً تلاش برای فرستادن پیام شروع شود - نه اینکه منتظر چرخه‌ی commit
    دوره‌ای بعدی (GIT_COMMIT_EVERY_SECONDS) بمانیم."""
    now = datetime.now(timezone.utc)
    today_str = now.strftime("%Y-%m-%d")
    if now.hour < DAILY_REPORT_HOUR_UTC:
        return

    git_commit_and_push()  # pull تازه - اگر یک اجرای دیگر گزارش را زده و پوش کرده ببینیمش
    ...
    today_trades = [h for h in trade_history if (h.get("closed_at") or "").startswith(today_str)]

    # شمارشِ «چند سیگنال همون روز صادر شده» - جدا از «چند تا همون روز بسته شده»
    # (عیناً همان منطق ربات VIP، توضیح کامل آنجا).
    issued_today_from_history = sum(1 for h in trade_history if (h.get("opened_at") or "").startswith(today_str))
    candle_states = state.get("candle_signals", {})
    open_today = sum(
        1 for s in candle_states.values()
        if (t := (s or {}).get("open_trade")) and not t.get("closed")
    ...
    if bot.send_telegram_message(msg):
        logger.info(f"📋 گزارش روزانه برای {today_str} ارسال شد ({len(today_trades)} معامله)")
    else:
        logger.error(f"📋 ارسال گزارش روزانه برای {today_str} ناموفق بود - امروز دیگر دوباره تلاش "
                      f"نمی‌شود (برای جلوگیری قطعی از ارسال دوبرابر)؛ لاگ‌ها را چک کنید.")
...
def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any], ema7_now: Optional[float]) -> List[Dict[str, Any]]:
    ...
    # 🔴 هر رویدادی که این تابع تشخیص بده - علاوه بر events (که فقط برای فرستادن پیام
    # همین لحظه استفاده می‌شه) - با timestamp دقیق در trade["events"] هم دائمی ثبت
    # می‌شه (عیناً الگوی candle_engine.py/ربات VIP)، تا log_trade_result بتونه کل این
    # آرایه رو در trade_history ذخیره کنه و مسیر حرکتیِ کاملِ سیگنال (نه فقط hit/fills
    # نهایی، بلکه دقیقاً کِی و روی چه قیمتی هر مرحله اتفاق افتاد) برای همیشه بمونه.
    def _persist(evs):
...
def send_trade_exit(bot: "TradeiscoolBot", symbol: str, display_name: str, tf_key: str, tf_label: str,
    ...
    # 🔴 رفعِ باگِ «پیام سیگنال‌های بسته‌شده دوباره ارسال می‌شود» - توضیح کامل بالای
    # تعریف _persist_immediately. اگر پیام واقعاً ارسال شد (sent=1)، پرچم‌های
    # trade["hit"]/["closed"] که همین بالا در حافظه ست شدند همین الان روی دیسک/ریموت
    # هم ثبت می‌شوند - نه اینکه تا ذخیره‌ی دوره‌ای بعدی (تا ۲ دقیقه) صبر کنیم.
    if sent:
        _persist_immediately(global_state, reason=f"خروج معامله {symbol} [{tf_key}] {event.get('type')}")
    return sent
...
def process_and_send_symbol_tf(bot: "TradeiscoolBot", fetch_fn, symbol: str, display_name: str,
    ...
    def _try_open_trade(sig):
        ...
        🔴 اضافه شد: چکِ DISABLED_AUTO_TIMEFRAMES - اگه این تایم‌فریم عمداً خاموش شده
        (طبق تصمیم کاربر بر اساس بک‌تست)، سیگنالِ خودکارِ جدید روی این تایم‌فریم اصلاً باز
        نمی‌شه. این چک فقط همین‌جا (باز کردنِ معامله‌ی *جدید*) اثر داره - معامله‌ی از قبل
        بازِ همین تابع (چند خط پایین‌تر، مستقل از _try_open_trade) دقیقاً طبق قبل رهگیری
        می‌شه."""
        if tf_key in DISABLED_AUTO_TIMEFRAMES:
            return None
        if global_state is not None and is_symbol_blocked_today(display_name, global_state):
            ...
            return None
        return open_new_trade(sig, global_state=global_state)
    ...
    def _send_entry(sig, trade, hist):
        ...
        if msg_id:
            ...
            sent += 1
            # 🔴 همان رفعِ باگِ بالا (توضیح کامل بالای _persist_immediately): معامله‌ی
            # تازه‌باز‌شده و signal_message_id (لازم برای ریپلای پیام‌های خروج بعدی) هم
            # همین الان ذخیره/پوش می‌شود - نه اینکه تا ذخیره‌ی دوره‌ای بعدی منتظر بماند و
            # با یک kill ناگهانی، این سیگنال کاملاً از دیسک/ریموت گم شود.
            _persist_immediately(global_state, reason=f"ورود جدید {symbol} [{tf_key}]")
        time.sleep(1.5)
    ...
    def _force_close(trade, candle, hist):
        ...
        trade.setdefault("events", []).append(_new_trade_event("opposite_signal", price=exit_price, r=exit_r))
        ...
        reply_id = trade.get("signal_message_id")
        sent_now = bot.send_telegram_photo(msg, None, reply_to_message_id=reply_id)
        if sent_now:
        ...
        if trade_history is not None:
            log_trade_result(trade_history, symbol, tf_key, source_label, trade)
        # 🔴 همان رفعِ باگِ بالا: اینجا trade["closed"]=True چند خط بالاتر ست شده - اگر
        # پیام واقعاً ارسال شد، همین الان ذخیره/پوش می‌شود تا اگر پروسه درست بعد از
        # این متوقف شود، دور بعدی دوباره همین بسته‌شدن را (با سیگنال مخالف بعدی) پیام ندهد.
        if sent_now:
            _persist_immediately(global_state, reason=f"بسته‌شدن اجباری {symbol} [{tf_key}]")
        time.sleep(1.5)
    ...
    if sym_state is None:
        ...
        for idx, k in enumerate(new_candles):
            ...
            if sig and idx == last_new_idx:
                ...
                # 🔴 رفع باگ «بسته‌شدن بدون هج و بازشدن مجدد هم‌جهت»: قبلاً اینجا با هر
                # سیگنال جدید (فارغ از جهتش) معامله‌ی باز فعلی force-close می‌شد و متنِ
                # «trend reversed, a new {opposite} signal is replacing this trade» ارسال
                # می‌شد - حتی وقتی سیگنالِ تازه دقیقاً هم‌جهت با معامله‌ی باز بود (که نه
                # واقعاً reversal است و نه متنِ پیام درست است). الان فقط زمانی force-close
                # و بازِ مجدد انجام می‌شود که سیگنالِ جدید واقعاً مخالفِ جهتِ معامله‌ی باز
                # باشد؛ اگر هم‌جهت بود، سیگنالِ تازه نادیده گرفته می‌شود و معامله‌ی باز فعلی
                # (که همان جهت را پیشنهاد می‌دهد) بدون وقفه ادامه پیدا می‌کند.
                if prev_trade and not prev_trade.get("closed"):
                    if sig["side"] != prev_trade["side"]:
                        _force_close(prev_trade, k, state["hist"])
                        trade = _try_open_trade(sig)
                        if trade:
                            state["open_trade"] = trade
                            _send_entry(sig, trade, state["hist"])
                    # else: سیگنالِ هم‌جهت با معامله‌ی باز فعلی - نادیده گرفته می‌شود
                else:
                    trade = _try_open_trade(sig)
                    if trade:
                        state["open_trade"] = trade
...
# متن اقدام برای هر تارگت - از shared_risk_config.py می‌آید (نه دیگه اینجا تعریف می‌شه).
# 🔴 رفعِ ریسکِ ناهماهنگی: قبلاً اینجا یک کپیِ دستیِ جداگانه بود که کلیدهاش شماره‌ی تارگت
# (۱ تا ۴) بود؛ نسخه‌ی مشترک با کلید = ضریب R (۱/۲/۴/۶، هماهنگ با RR_TARGETS) تعریف شده -
# چون در candle_engine.py هم لوکیشن‌های "level" همیشه ضریب R هستن. برای همین، هر جا این
# فایل بر اساس شماره‌ی تارگتِ محلی‌اش (lvl["n"]) به این دیکشنری نگاه می‌کنه، باید اول با
# lvl["r"] به کلید مشترک تبدیل کنه (نگاه کنید محل استفاده در format_rr_exit_message).
...
def _id_hashtag(sid: str) -> str:
    """نسخه‌ی هشتگِ یک signal_id - عیناً معادل _id_hashtag در candle_engine.py (ربات مرجع):
    تلگرام در هشتگ فقط حروف/عدد/underscore رو می‌پذیره (نه خط‌تیره)، پس «ALT-004821» به
    «#ALT_004821» تبدیل می‌شه."""
    return re.sub(r"[^A-Za-z0-9_]", "_", sid)


def _id_line(trade: Dict[str, Any]) -> str:
    """خط هشتگِ شناسه‌ی سیگنال («#ALT_004821») - طبق درخواست صریح کاربر در همین گفتگو اضافه
    شد (برخلاف بقیه‌ی خطوط اضافه‌ای که قبلاً عمداً از این پیام‌ها حذف شدند - توضیح بالای
    format_entry_message)، چون این یک استثنای مشخص و هدفمند است: بدون این هشتگ، وقتی این
    سیگنال به کانال VIP هم کپی می‌شود، هیچ راهی برای اشاره‌ی دقیق به همین یک سیگنال (نه یک
    سیگنال دیگر روی همان نماد) وجود ندارد - نه برای کاربر، نه برای ادمین. اگر شناسه موجود
    نباشد (رکورد خیلی قدیمی)، رشته‌ی خالی برمی‌گرداند - هیچ خط اضافه‌ای اضافه نمی‌شود.

    با تپ‌کردن روی این هشتگ در اپ تلگرام، تمام پیام‌های همین یک سیگنال (ورود + هر
    تارگت/استاپ) در یک نتیجه‌ی جستجوی داخل کانال جمع می‌شن. subscription_bot.py (ربات
    مشترکینِ کانال VIP) دقیقاً همین هشتگ رو با یک regex می‌خونه تا وقتی این پیام به کانال VIP
    رله می‌شه، شناسه‌ی واقعیِ این معامله («ALT-004821») - نه یک شناسه‌ی تازه‌ساخته‌شده‌ی
    داخلی - رو به‌عنوان signal_id همون معامله در سیستم VIP هم ثبت کنه؛ بدون این هشتگ،
    /admin_close_trade یا /signal روی سیستم VIP هیچ‌وقت این سیگنال رو پیدا نمی‌کردن.

    🔴 طبق درخواست صریح، دو تغییر نسبت به نسخه‌ی قبلی: (۱) دیگه خودِ متنِ خام شناسه («🆔
    ALT-004821») نمایش داده نمی‌شه - چون کاربردی نداشت و فقط هشتگ («#ALT_004821») کافیه؛
    subscription_bot.py هم الان مستقیماً از روی همین هشتگ (نه یک خط 🆔 جدا) شناسه رو استخراج
    می‌کنه. (۲) این هشتگ دیگه بالای پیام (زیر خط تیتر) نیست - به‌جاش زیرِ خط تاریخ/زمان و
    درست بالای فوتر ریسک منتقل شده - نگاه کنید محل فراخوانیِ این تابع در هر format_*_message.

    ⚠️ نکته‌ی فنی: یک \n در ابتدا (نه انتها) برمی‌گردونه، هماهنگ با همون تصمیم در
    candle_engine.py - چون در محل فراخوانی، بلافاصله بعدش دو \n دیگر (فاصله‌ی خط خالی قبل از
    SIGNAL_FOOTER) لیترال اضافه می‌شه؛ اگه این تابع خودش هم \n انتها می‌ذاشت، جمع می‌شد به یک
    خط خالیِ اضافه‌ی ناخواسته."""
...
def format_rr_exit_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    body = (
        ...
        # 🔴 TARGET_ACTION_LINE (از shared_risk_config.py) با کلید = ضریب R تعریف شده،
        # نه شماره‌ی تارگت محلی این فایل - پس اینجا با lvl_cfg["r"] تبدیل می‌شه.
        f"{TARGET_ACTION_LINE.get(lvl_cfg['r'], '')}\n\n"
...
def check_stale_open_trades(state: Dict[str, Any]) -> None:
    """طبق درخواست صریح («همه‌ی مواردی که انجام دادی را روی این بات هم انجام بده») -
    عیناً معادل check_stale_open_trades در candle_engine.py (ربات VIP): اگر یک معامله
    بیش از STALE_OPEN_TRADE_HOURS ساعت باز بماند بدون بسته‌شدن، به ادمین هشدار می‌دهد
    - چون این تقریباً همیشه یعنی یک قیمت/کندلِ رصدشده به هر دلیلی گم شده، نه اینکه
    معامله واقعاً هنوز باز باشد.

    ⚠️ تفاوت مهم با نسخه‌ی ربات VIP: آنجا این هشدار دکمه‌های inline (✅ بستن/❌ ضرر
    اجباری/⚪ بریک‌ایون اجباری) هم دارد، چون subscription_bot.py یک پروسه‌ی همیشه-روشن
    با getUpdates پیوسته است که می‌تواند فوراً کلیک روی آن دکمه‌ها را دریافت و پردازش
    کند. این بات (bot.py) چنین پروسه‌ای ندارد - فقط پیام می‌فرستد، هرگز پاسخ/دستور
    دریافت نمی‌کند. اضافه‌کردن دکمه‌هایی که هیچ‌وقت واقعاً کار نمی‌کنند (چون هیچ‌کس
    callback_query‌شان را نمی‌خواند) گمراه‌کننده‌تر از نداشتن دکمه است - ادمین فکر
    می‌کند با یک کلیک حل می‌شود، در حالی که هیچ اتفاقی نمی‌افتد. به‌جایش، این هشدار
    تمام جزییات لازم برای بررسی دستی (شناسه/نماد/جهت/ورود/استاپ/تارگت‌های رسیده/مجموع
    R بانک‌شده تا الان/سن) را می‌دهد، و توضیح می‌دهد چرا این‌جا فعلاً باید دستی رسیدگی
    شود - نه یک دکمه‌ی تزئینیِ بی‌اثر."""
    ...
    for key, s in candle_states.items():
        ...
        signal_id = t.get("signal_id", key)
        hit_labels = [f"T{lvl['n']}" for lvl in TARGET_LEVELS if t.get("hit", {}).get(str(lvl["n"]))]
        hit_part = "/".join(hit_labels) if hit_labels else "none"
        # ⚠️ برخلاف نسخه‌ی VIP، اینجا از عبارت «Fair-value result if closed now» استفاده
        # نمی‌شود - چون compute_final_r این بات فقط مجموع fills واقعاً رخ‌داده را جمع
        # می‌زند (نه یک تخمین کامل شامل بخش هنوز-باز)، پس این عدد صرفاً «R بانک‌شده تا
        # الان» است، نه نتیجه‌ی کامل اگر همین الان بسته شود - عبارت دقیق‌تر برای
        # جلوگیری از برداشت اشتباه استفاده شده.
        banked_r = round(compute_final_r(t), 2)
        opened_at_str = (datetime.fromtimestamp(opened_at_ms / 1000, tz=timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
                          if opened_at_ms else "?")
        symbol, tf_key = (key.split("|", 1) + ["?"])[:2] if "|" in key else (key, "?")
        tf_label = TIMEFRAMES.get(tf_key, {}).get("label", tf_key)
        text = (
            f"⚠️ Stuck open trade — needs review\n\n"
            f"🆔 {signal_id}\n"
            f"{symbol} {t.get('side', '?')} [{tf_label}]\n"
            f"Entry: {t.get('entry', '?')}  ·  Current stop: {t.get('working_sl', t.get('sl', '?'))}\n"
            f"Targets hit: {hit_part}{' · Runner trailing' if t.get('trailing_activated') else ''}\n"
            f"R banked so far: {banked_r:+.2f}R\n"
            f"Opened: {opened_at_str}  ·  Open for {age_hours}h (threshold {STALE_OPEN_TRADE_HOURS}h)\n\n"
            f"Likely stuck because a price/candle update was missed, not because it's genuinely "
            f"still open. This bot has no live command interface, so please check the real price "
            f"manually against entry/stop/targets. If this same signal was also relayed to the VIP "
            f"channel, it can be resolved there instantly with /admin_close_trade {signal_id} "
            f"[final_r] (that system tracks its own live copy and supports one-click closing)."
        )
        notify_admin(text, state, kind=f"stale_open_trade:{signal_id}")
        ...
        try:
            ...
            for coin in top_coins:
                try:
                    ...
                    if volume_24h < TREND_ALTS_MIN_VOLUME_USD:  # فیلتر نقدشوندگی
                        continue
                    # 🔴 اضافه شد: فیلتر market cap - جدا از فیلتر حجم بالا، چون حجم به‌تنهایی
                    # میم‌کوین‌های در حالِ پامپ (حجمِ لحظه‌ای بالا، market cap واقعی پایین) رو
                    # رد نمی‌کنه؛ این دقیقاً همون نوع دارایی‌ایه که طبق مشاهده‌ی کاربر نتیجه‌ی
                    # ضعیف داده.
                    market_cap = quote_data.get('market_cap', 0) or 0
                    if market_cap < TREND_ALTS_MIN_MARKET_CAP_USD:
                        continue
...
def run_scan_cycle(state: Dict[str, Any]) -> None:
    ...
    اسکن جریان استیبل‌کوین (هر کدام فقط اگر due باشند). این تابع هر
    ...
    # 🆕 گزارش روزانه‌ی خودکار - قبلاً اینجا کاملاً حذف شده بود (طبق تصمیم قبلی کاربر،
    # چون معماریِ قدیمی‌اش ذاتاً شکننده بود - جزییات کامل بالای تعریفِ خودِ
    # maybe_send_daily_report). طبق درخواست صریح این‌بار («دقیقاً مانند کانال VIP هر
    # روز در همان زمان فقط یک مرتبه») با یک معماریِ اساساً متفاوت و ایمن (پرچمِ ددوپ
    # در یک فایل کاملاً جداگانه، بیرون از چرخه‌ی backup/revert خودِ state) دوباره
    # اضافه شد - همان الگویی که از اول برای candle_engine.py (ربات VIP) درست کار
    # می‌کرد. try/except اینجا هم دقیقاً همان دلیل بقیه‌ی try/exceptهای این تابع را
    # دارد: اگر خودِ ارسال گزارش (مثلاً یک قطعیِ موقتِ تلگرام) خطا بدهد، نباید کل
    # run_scan_cycle (و state این دور) را قربانی کند.
    try:
        maybe_send_daily_report(bot, state)
    except Exception as e:
        logger.error(f"❌ maybe_send_daily_report خطا داد - دور اسکن ادامه می‌دهد: {e}")

    # 🆕 هشدار «معامله‌ی گیرکرده» - عیناً معادل check_stale_open_trades در
    # candle_engine.py (ربات VIP)، توضیح کامل بالای تعریف خودِ تابع (از جمله اینکه چرا
    # اینجا برخلاف ربات VIP دکمه‌ی inline ندارد).
    try:
        check_stale_open_trades(state)
    except Exception as e:
        logger.error(f"❌ check_stale_open_trades خطا داد - دور اسکن ادامه می‌دهد: {e}")

    bot.sync_state()
    # 🔴 این کش صرفاً یک بهینه‌سازی (رد سریع‌تر نمادهای شناخته‌شده‌ی بدون‌پشتیبانی) است -
    ...
    # نمی‌دهیم آن خطا کل تابع را متوقف کند.
    try:
...
def main():
    ...
    # 🔴 چون bot.py و shared_risk_config.py در دو ریپوی گیت جداگانه‌اند، این لاگ روشن
    # می‌کنه که آیا گاردِ ضدِ drift همین الان واقعاً فعاله (یعنی یک قدمِ CI جداگانه
    # shared_risk_config.py رو از ریپوی دیگه fetch کرده) یا این ربات فقط با مقادیرِ
    # fallbackِ محلی (که دستی باید هماهنگ نگه داشته بشن) داره اجرا می‌شه.
    if _SHARED_RISK_CONFIG_AVAILABLE:
        logger.info("✅ shared_risk_config.py پیدا شد - گاردِ ضدِ drift در برابر ریپوی دیگر فعال است")
    else:
        logger.warning("⚠️ shared_risk_config.py در این ریپو پیدا نشد - از مقادیرِ fallbackِ محلی "
                        "(کپیِ دستیِ هم‌ارز) استفاده می‌شود؛ گاردِ خودکارِ ضدِ drift غیرفعال است تا وقتی "
                        "یک قدمِ CI این فایل را از ریپوی دیگر fetch کند.")
