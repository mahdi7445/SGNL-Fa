...
def run_scan_cycle(state: Dict[str, Any]) -> None:
    ...
    # 🔴🔴 حذفِ کاملِ سیستم «گزارش روزانه‌ی خودکار» (طبق درخواست صریح کاربر): با
    # اینکه چند لایه‌ی دفاعی پشت‌سرهم (pull→mark→push→send، بازیابی از دیسک در برابر
    # revert، و غیره) برای رفعِ «گزارش چند بار در روز/در ساعت‌های مختلف تکرار
    # می‌شود» اضافه شده بود، این باگ همچنان به‌طور متناوب رخ می‌داد - چون معماری
    # زیرین (یک پرچمِ تک "امروز فرستاده شد یا نه" که باید همزمان با موفقیتِ واقعیِ
    # ارسال پیام در تلگرام synced بماند، آن‌هم در یک پروسه که هر چند ساعت یک‌بار
    # kill/timeout/crash می‌شود و از صفر با state رفرش‌شده از گیت‌هاب دوباره
    # می‌آید) ذاتاً شکننده است - هر گپ زمانی، هرچقدر کوچک، بین mark و push (یا بین
    # push و بعدش خواندنِ state توسط اجرای بعدی) دوباره باعث تکرار می‌شود؛ برخلاف
    # `subscription_bot.py` («ربات VIP») که یک پروسه‌ی همیشه-روشن است و دستور
    # `/results` را فقط لحظه‌ای و درخواستی (نه زمان‌بندی‌شده و نه در برابر
    # crash/restart) پاسخ می‌دهد - معماری‌ای که این‌جا (اسکریپت GitHub Actions با
    # اجراهای متناوب) قابل تکرار نیست. طبق تصمیم کاربر، به‌جای یک رفعِ ناقصِ دیگر،
    # کل مسیرِ ارسالِ خودکارِ این گزارش (RESULTS_REPORT_HOUR، is_results_report_due،
    # mark_results_report_processed، format_results_message و توابع آماریِ کمکی‌اش)
    # کاملاً حذف شد. رصد و ثبتِ نتیجه‌ی هر معامله (log_trade_result/log_target_event،
    # داخل trade_history/target_events در state.json) دست‌نخورده باقی ماند - فقط
    # دیگر هیچ پیامی خودکار از رویشان ساخته/فرستاده نمی‌شود؛ هیچ بخش دیگر ربات
    # (تشخیص سیگنال، حد ضرر/تارگت‌ها، پیام‌های ورود/خروج) تحت تأثیر قرار نگرفت.
