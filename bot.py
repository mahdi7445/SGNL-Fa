...
# پیش‌فرض «1m» است (طبق تصمیم صریح کاربر)؛ برای برگردوندنش، فقط کافیه این secret رو حذف
# کنید یا مقدارش رو خالی بذارید.
# 🔴 رفعِ باگ: قبلاً اینجا os.environ.get("DISABLED_AUTO_TIMEFRAMES") بدون آرگومان دومِ
# پیش‌فرض بود - یعنی خلاف همین کامنتِ بالا (که می‌گفت پیش‌فرض باید "1m" باشه) و برخلافِ
# candle_engine.py (که os.environ.get("DISABLED_AUTO_TIMEFRAMES", "1m") داره)، اگر این
# متغیر محیطی/secret اصلاً در ریپو تنظیم نشده بود، os.environ.get بدون پیش‌فرض مقدار
# None برمی‌گردوند و همون خط بعدی (.split(",")) با AttributeError کرش می‌کرد - یعنی ربات
# اصلاً بالا نمی‌اومد، نه اینکه فقط تایم‌فریم پیش‌فرضِ مستندشده رو نادیده بگیره.
DISABLED_AUTO_TIMEFRAMES = set(
    tf.strip().lower() for tf in (os.environ.get("DISABLED_AUTO_TIMEFRAMES") or "1m").split(",") if tf.strip()
)
