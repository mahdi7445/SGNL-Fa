...
def format_results_message(trade_history: List[Dict[str, Any]], now_utc: Optional[datetime] = None) -> Optional[str]:
    ...
    بزرگ‌تر و کمتر خوانا شود). اگر امروز هنوز هیچ معامله‌ای بسته نشده None
    برمی‌گرداند (چیزی برای گزارش نیست).

    طبق درخواست کاربر: حالا مشخص می‌کند چند معامله به هر تارگت (T1..T4) رسیده و
    چند رانر با موفقیت توسط تریلینگ‌استاپ بسته شده - نه فقط برد/باخت خام."""
    if now_utc is None:
    ...
    target_lines = [
        f"  T{lvl['n']} ({lvl['r']}R, {lvl['close_frac']*100:.0f}%): {stats['target_hit_counts'][lvl['n']]} reached"
        for lvl in TARGET_LEVELS
    ]
    runner_line = (
        f"  Runner (final {RUNNER_FRACTION*100:.0f}%, trailing {TRAILING_DISTANCE_R}R): "
        f"{stats['runner_count']} closed" +
        (f" · avg {stats['runner_avg_r']:+.2f}R" if stats['runner_avg_r'] is not None else "")
    )

    return (
        f"📊 DAILY SIGNAL PERFORMANCE — {today_str}\n"
        f"<i>{stats['total']} trades closed today - every signal sent, no cherry-picking</i>\n\n"
...
def format_entry_message(symbol: str, display_name: str, timeframe_label: str, sig: Dict[str, Any],
    ...
    # طبق درخواست کاربر: در همان پیام سیگنال ورود، دقیقاً مشخص است چند درصد پوزیشن
    # در هر تارگت بسته می‌شود و چند درصد به‌عنوان رانر با تریلینگ باقی می‌ماند.
    targets_lines = "\n".join(
        f"🎯 {TARGET_LABELS[lvl['n']]}: {format_signal_price(trade['entry'] + sign * lvl['r'] * trade['r'])} "
        f"→ close {lvl['close_frac']*100:.0f}%"
        for lvl in TARGET_LEVELS
    )
    body = (
        ...
        f"{targets_lines}\n"
        f"🏁 Runner (final {RUNNER_FRACTION*100:.0f}%): trails {TRAILING_DISTANCE_R}R after Target 4, no fixed target"
    )
    if extra_line:
        body += f"\n{extra_line}"
...
def run_scan_cycle(state: Dict[str, Any]) -> None:
    ...
    if is_results_report_due(now_utc, state):
        ...
        report_msg = format_results_message(trade_history, now_utc)
        if report_msg:
