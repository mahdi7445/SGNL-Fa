...
MAX_SCAN_SECONDS = 600   # سقف کل اسکن کندلی (۱۰ دقیقه) - قبل از اجرای بعدیِ کرون (هر ۱۵ دقیقه) با فاصله‌ی امن متوقف می‌شود
...
# طبق درخواست: فرمت همه‌ی سیگنال‌های کندلی دقیقاً مثل candle_engine.py (تارگت + عکس
# چارت) باشد. منطق زیر عیناً از همان فایل پورت شده است.
...
RR_TARGETS = [1, 2, 3, 4]
TARGET_LABELS = {1: "تارگت ۱", 2: "تارگت ۲", 3: "تارگت ۳", 4: "تارگت ۴"}
HIST_KEEP = 80                # تعداد کندل نگه‌داشته‌شده برای رسم چارت (به‌جای فقط ۲ کندل قبلی)

# وزن هر مرحله از حجم اصلی پوزیشن (طبق مدیریت پوزیشن candle_engine.py)
#   تارگت ۱ (R=1): بستن ۵۰٪ حجم پوزیشن
#   تارگت ۲ (R=2): بستن ۵۰٪ از باقی‌مانده + انتقال حد ضرر به نقطه‌ی ورود (ریسک‌فری)
#   تارگت ۳ (R=3): بستن ۷۰٪ از باقی‌مانده + شروع تریل‌کردن حد ضرر
#   تارگت ۴ (R=4): بستن ۸۵٪ از باقی‌مانده + ۱۵٪ باقی («رانر») با همان تریلینگ‌استاپ باز می‌ماند
_W1, _W2, _W3 = 0.5, 0.25, 0.175
_REMAINDER_AT_T4 = 1 - _W1 - _W2 - _W3
_RUNNER_KEEP_FRACTION = 0.15
_W4 = _REMAINDER_AT_T4 * (1 - _RUNNER_KEEP_FRACTION)
_W_RUNNER = _REMAINDER_AT_T4 * _RUNNER_KEEP_FRACTION

CLOSE_TYPE_LABELS_FA = {
    "stop": "استاپ خورد (ضرر کامل)",
    "breakeven": "ریسک‌فری بسته شد (تارگت ۱-۲ بانک شده، بقیه بدون ضرر)",
    "trailing_stop": "تریلینگ‌استاپ خورد (تارگت ۱-۳ بانک شده)",
    "runner_stop": "رانر بسته شد (هر ۴ تارگت + رانر تریلینگ)",
    "opposite_signal": "زودتر بسته شد - سیگنال مخالف صادر شد",
}
...
# گزارش روزانه‌ی خودکار عملکرد سیگنال‌ها (چون bot.py برخلاف subscription_bot.py پیوسته
# اجرا نمی‌شود، نمی‌تواند دستور /results را بشنود - در عوض هر روز یک‌بار خودش می‌فرستد)
RESULTS_REPORT_HOUR = 8  # ساعت ۸ UTC (تهران ~۱۱:۳۰ صبح)
...
def compute_final_r(trade: Dict[str, Any]) -> float:
    ...
    if not hit["1"]:
        return -1.0
    if not hit["2"]:
        return _W1 * 1 + (1 - _W1) * (-1.0)
    if not hit["3"]:
        return _W1 * 1 + _W2 * 2 + (1 - _W1 - _W2) * 0.0
    if not hit["4"]:
        return _W1 * 1 + _W2 * 2 + _W3 * 3 + _REMAINDER_AT_T4 * 3.0
    return _W1 * 1 + _W2 * 2 + _W3 * 3 + _W4 * 4.0 + _W_RUNNER * 4.0


# ================== آمار عملکرد سیگنال‌ها (مشترک با نتیجه‌ای که کاندل_انجین ذخیره می‌کند) ==================
...
def log_trade_result(trade_history: List[Dict[str, Any]], symbol: str, tf_key: str,
    ...
    """وقتی معامله بسته می‌شود (استاپ/ریسک‌فری/تریلینگ/رانر/سیگنال مخالف)، نتیجه‌ی شفاف
    آن (چطور بسته شده + چند R) را برای آمار برد/باخت ذخیره می‌کند - چیزی که کم بود و
    الان با گزارش روزانه‌ی خودکار در کانال قابل دیدن است."""
    ...
    trade_history.append({
        ...
        "close_reason": CLOSE_TYPE_LABELS_FA.get(close_type, close_type),
...
def format_results_message(trade_history: List[Dict[str, Any]]) -> Optional[str]:
    """گزارش روزانه‌ی خودکار عملکرد سیگنال‌ها - چند معامله به تارگت رسیده، چند تا
    استاپ خورده، بر حسب نماد و تایم‌فریم. اگر هنوز هیچ معامله‌ای بسته نشده None
    برمی‌گرداند (چیزی برای گزارش نیست)."""
    stats = compute_stats(trade_history)
    if not stats:
        return None

    by_symbol = compute_stats_by_group(trade_history, "symbol")
    symbol_lines = [
        f"  {name}: {s['total']} معامله · {s['win_rate']:.0f}٪ برد · {s['total_r']:+.1f}R"
        for name, s in sorted(by_symbol.items(), key=lambda kv: -kv[1]["total"])[:15]
    ]

    tf_order = ["15m", "1h", "4h", "1d", "1w"]
    by_tf = compute_stats_by_group(trade_history, "tf")
    tf_lines = [
        f"  {name}: {s['total']} معامله · {s['win_rate']:.0f}٪ برد · {s['total_r']:+.1f}R"
    ...
    return (
        f"📊 <b>گزارش روزانه‌ی عملکرد سیگنال‌ها</b>\n"
        f"<i>{stats['total']} معامله‌ی بسته‌شده‌ی اخیر - همه‌ی سیگنال‌های ارسالی، بدون گزینشی</i>\n\n"
        f"✅ برد: {stats['wins']} ({stats['win_rate']:.0f}٪)\n"
        f"⚪ ریسک‌فری: {stats['breakeven']}\n"
        f"❌ ضرر: {stats['losses']}\n\n"
        f"مجموع: <b>{stats['total_r']:+.1f}R</b>\n"
        f"میانگین هر معامله: <b>{stats['avg_r']:+.2f}R</b>\n\n"
        f"<b>بر اساس نماد</b>\n" + "\n".join(symbol_lines) + "\n\n"
        f"<b>بر اساس تایم‌فریم</b>\n" + "\n".join(tf_lines)
    )


def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any], ema7_now: Optional[float]) -> List[Dict[str, Any]]:
    """
    وضعیت معامله‌ی باز را با یک کندل تازه‌بسته‌شده چک می‌کند. برخلاف candle_engine.py،
    اینجا نسخه‌ی «قیمت زنده‌ی بین دو کندل» (REST/WebSocket) عمداً پیاده نشده - چون
    bot.py هر ۱۵ دقیقه یک‌بار (نه پیوسته) اجرا می‌شود و افزودن آن یعنی درخواست‌های
    زنده‌ی اضافه در هر اجرا که سهمیه‌ی روزانه‌ی Twelve Data را کاملاً پر می‌کرد. در
    عوض همین بررسیِ مبتنی بر high/low کندل (رایگان، چون همان کندلی‌ست که برای تشخیص
    سیگنال هم‌اکنون دریافت شده) دقت کافی می‌دهد؛ فقط کشف خروج ممکن است تا بسته‌شدن
    کندل بعدی (نه لحظه‌به‌لحظه) به تعویق بیفتد.
    """
...
def build_chart_from_hist(hist: List[Dict[str, Any]], title: str,
                           trade: Optional[Dict[str, Any]] = None) -> Optional[str]:
    if len(hist) < 15:
        return None
...
def process_and_send_symbol_tf(bot: "TradeiscoolBot", fetch_fn, symbol: str, display_name: str,
                                ...
                                extra_line: Optional[str] = None) -> Tuple[Optional[Dict[str, Any]], int, Optional[str]]:
    """
    ...
    def _send_exit(trade, event, hist):
        ...
        if event["type"] == "stop":
            ...
            title_suffix = f"T{event['level']}"
        chart = build_chart_from_hist(hist, f"{display_name} {tf_label} · {title_suffix}", trade=trade)
        if event["type"] == "stop":
            msg = format_stop_message(symbol, display_name, tf_label, trade, event, source_label)
        elif event["type"] == "breakeven":
            msg = format_breakeven_message(symbol, display_name, tf_label, trade, event, source_label)
        elif event["type"] == "trailing_stop":
            msg = format_trailing_stop_message(symbol, display_name, tf_label, trade, event, source_label)
        elif event["type"] == "runner_stop":
            msg = format_runner_stop_message(symbol, display_name, tf_label, trade, event, source_label)
        else:
...
TARGET_ACTION_LINE = {
    1: "✂️ ۵۰٪ از پوزیشن را همین‌جا ببندید.",
    2: "✂️ ۵۰٪ از باقی‌مانده را ببندید و حد ضرر را به نقطه‌ی ورود منتقل کنید (ریسک‌فری).",
    3: "✂️ ۷۰٪ از باقی‌مانده را ببندید و حد ضرر را شروع به تریل‌کردن کنید.",
    4: "✂️ ۸۵٪ از باقی‌مانده را ببندید. ۱۵٪ باقی («رانر») با همان تریلینگ‌استاپ باز بماند تا "
       "اگر حرکت بزرگ‌تر از ۴R هم رخ داد، بخشی از سودش گرفته شود.",
}


def _signal_desc(timeframe_label: str, source_label: Optional[str]) -> str:
...
def format_entry_message(symbol: str, display_name: str, timeframe_label: str, sig: Dict[str, Any],
    ...
    side_label = "خرید" if sig["side"] == "BUY" else "فروش"
    confirm_txt = "✅ تأییدشده (اینگولفینگ/پین‌بار)" if sig["confirmed"] else "بدون تأیید اضافه"
    header = format_signal_header(emoji, side_label, display_name, symbol, _signal_desc(timeframe_label, source_label))

    sign = 1 if sig["side"] == "BUY" else -1
    targets_lines = "\n".join(
        f"🎯 {TARGET_LABELS[t]}: {format_price(trade['entry'] + sign * t * trade['r'])}" for t in RR_TARGETS
    )
    body = (
        f"ورود: {format_price(sig['price'])}\n"
        f"❌ حد ضرر: {format_price(trade['sl'])}\n"
        f"{targets_lines}\n"
        f"تایید: {confirm_txt}"
    )
    if extra_line:
...
def format_rr_exit_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    header = format_signal_header("✅", f"{label} فعال شد", display_name, symbol,
                                   _signal_desc(timeframe_label, source_label))
    body = (
        f"{label} روی این معامله‌ی {direction} رسید.\n"
        f"ورود: {format_price(trade['entry'])}  ·  الان: {format_price(event['price'])}\n\n"
...
def format_stop_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
                         event: Dict[str, Any], source_label: Optional[str] = None) -> str:
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    header = format_signal_header("❌", "استاپ خورد", display_name, symbol, _signal_desc(timeframe_label, source_label))
    body = (
        f"حد ضرر روی این معامله‌ی {direction} فعال شد - کل باقی‌مانده‌ی پوزیشن بسته شد.\n"
        f"ورود {format_price(trade['entry'])}  ·  استاپ {format_price(event['price'])}\n\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    return f"{header}\n\n{body}\n\n{SIGNAL_FOOTER}"
...
def format_breakeven_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
                              event: Dict[str, Any], source_label: Optional[str] = None) -> str:
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    header = format_signal_header("⚪", "ریسک‌فری بسته شد", display_name, symbol,
                                   _signal_desc(timeframe_label, source_label))
    body = (
        f"قیمت به نقطه‌ی ورود این معامله‌ی {direction} برگشت - باقی‌مانده‌ی پوزیشن بدون ضرر بسته شد "
        f"(۵۰٪ قبلاً در تارگت ۱ و ۲ بانک شده بود).\n"
        f"ورود: {format_price(trade['entry'])}\n\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    return f"{header}\n\n{body}\n\n{SIGNAL_FOOTER}"


def format_trailing_stop_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
                                  event: Dict[str, Any], source_label: Optional[str] = None) -> str:
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    header = format_signal_header("🔒", "تریلینگ‌استاپ خورد", display_name, symbol,
                                   _signal_desc(timeframe_label, source_label))
    body = (
        f"تریلینگ‌استاپ روی این معامله‌ی {direction} فعال شد - باقی‌مانده‌ی پوزیشن با سود قفل‌شده بسته شد.\n"
        f"ورود {format_price(trade['entry'])}  ·  بسته‌شده در {format_price(event['price'])}\n\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    return f"{header}\n\n{body}\n\n{SIGNAL_FOOTER}"
...
def format_runner_stop_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
                                event: Dict[str, Any], source_label: Optional[str] = None) -> str:
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    header = format_signal_header("🏁", "رانر بسته شد", display_name, symbol, _signal_desc(timeframe_label, source_label))
    body = (
        f"هر ۴ تارگت این معامله‌ی {direction} قبلاً بانک شده بود - همین الان بخش ۱۵٪ باقی‌مانده (رانر) "
        f"هم با تریلینگ‌استاپش بسته شد. معامله کامل شد.\n"
        f"ورود {format_price(trade['entry'])}  ·  رانر بسته‌شده در {format_price(event['price'])}\n\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    return f"{header}\n\n{body}\n\n{SIGNAL_FOOTER}"
...
def format_forced_close_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    direction = "خرید" if trade["side"] == "BUY" else "فروش"
    opposite = "فروش" if trade["side"] == "BUY" else "خرید"
    result_r = compute_final_r(trade)
    result_word = "سود" if result_r > 0 else ("ریسک‌فری" if result_r == 0 else "ضرر")
    header = format_signal_header("⚠️", "معامله بسته شد", display_name, symbol,
                                   _signal_desc(timeframe_label, source_label))
    body = (
        f"این یک هج نیست - روند برگشته و سیگنال {opposite} جدیدی جای این معامله‌ی {direction} را می‌گیرد. "
        f"برای اینکه چیزی مبهم نماند، همین الان با قیمت بازار بسته شد.\n"
        f"ورود {format_price(trade['entry'])}  ·  بسته‌شده در {format_price(exit_price)}\n"
        f"نتیجه: تقریباً {result_r:+.2f}R ({result_word})\n\n"
        f"👉 سیگنال جدید برای این برگشت، بلافاصله بعد از این پیام می‌آید.\n"
        f"⏰ {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
    )
    return f"{header}\n\n{body}\n\n{SIGNAL_FOOTER}"
    ...
    for tf in due_timeframes:
        ...
        for symbol, display_name in WATCHLIST_SYMBOLS.items():
            ...
            try:
                ...
                new_state, sent, err = process_and_send_symbol_tf(bot, fetch_fn, symbol, display_name, tf, tf_cfg, sym_state, trade_history=trade_history)
                ...
                for cmc_symbol in top30_symbols:
                    ...
                    try:
                        ...
                        new_state, sent, err = process_and_send_symbol_tf(
                            ...
                            trade_history=trade_history, source_label="۳۰ ارز برتر بازار")
                        if new_state is None:
...
def main():
    ...
    if is_results_report_due(now_utc, state):
        ...
        report_msg = format_results_message(trade_history)
        if report_msg:
