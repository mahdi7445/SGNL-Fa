...
TIMEFRAMES = {
    "5m":  {"td_interval": "5min",  "bar_seconds": 5 * 60,          "label": "۵ دقیقه‌ای"},
...
def is_timeframe_due(tf_key: str, now_utc: datetime) -> bool:
    """
    تعیین می‌کند آیا الان زمانِ چک‌کردن این تایم‌فریم هست یا نه.
    فرض: این تابع هر ۱۵ دقیقه یک‌بار (طبق cron ورک‌فلو) صدا زده می‌شود.
    """
    if FORCE_RUN_ALL:
        return True
    minute, hour = now_utc.minute, now_utc.hour
    if tf_key in ("5m", "15m"):
        return True                                   # هر اجرا (هر ۱۵ دقیقه)
    if tf_key == "1h":
        return minute < 15                             # فقط اولین اجرای بعد از هر ساعت
    if tf_key == "4h":
        return hour in CANDLE_SCAN_HOURS and minute < 15
    if tf_key == "1d":
        return hour == 0 and minute < 15
    if tf_key == "1w":
        return now_utc.weekday() == 0 and hour == 0 and minute < 15   # دوشنبه = شروع هفته
    return False
...
def is_top30_due(now_utc: datetime) -> bool:
    if FORCE_RUN_ALL:
        return True
    return now_utc.hour in TOP30_SCAN_HOURS and now_utc.minute < 15
...
def format_candle_signal_message(symbol: str, sig: Dict[str, Any], display_name: Optional[str] = None,
                                  timeframe_label: str = "۴ ساعته") -> str:
    side = sig["side"]
...
<b>مبتنی بر اندیکاتور کندلی - تایم‌فریم {timeframe_label}</b>
...
def candle_signal_scan_job(bot: "TradeiscoolBot", state: Dict[str, Any], now_utc: datetime) -> List[Dict[str, Any]]:
    ...
    due_timeframes = [tf for tf in TIMEFRAMES if is_timeframe_due(tf, now_utc)]
    ...
    logger.info(f"🔎 در حال بررسی {len(WATCHLIST_SYMBOLS)} نماد × {len(due_timeframes)} تایم‌فریم = {total_combos} مورد از Twelve Data...")
                ...
                if new_state is None:
                    ...
                    for sig in signals:
                        ...
                        if bot.send_telegram_message(message):
                            ...
                            time.sleep(1.2)
                time.sleep(8)  # رعایت محدودیت نرخ درخواست Twelve Data (۸ درخواست در دقیقه در پلن رایگان)
            except Exception as e:
                entry["error"] = str(e)
    ...
    if TOP30_ENABLED and is_top30_due(now_utc):
        if not TWELVEDATA_API_KEY:
                        ...
                        if new_state is None:
                            ...
                            for sig in signals:
                                ...
                                if bot.send_telegram_message(message):
                                    ...
                                    entry["signal"] = sig["side"]
                                    time.sleep(1.2)
                        time.sleep(8)  # رعایت محدودیت نرخ درخواست Twelve Data
                    except Exception as e:
                        entry["error"] = str(e)
...
def main():
    ...
    run_flow_perf = FORCE_RUN_ALL or (hour in SCAN_SCHEDULE_HOURS and now_utc.minute < 15)
