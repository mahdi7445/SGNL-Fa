...
LIVE_CHECK_INTERVAL_SECONDS = 15 * 60  # رهگیری REST زنده (فقط طلا) هر ۱۵ دقیقه، فقط برای معاملات باز
MAX_LIVE_CHECKS_PER_RUN = 6            # سقف تعداد چک زنده در هر دور، برای محافظت از بودجه
...
CLOSE_TYPE_LABELS_EN = {
    "stop": "Stop hit (full loss)",
    "breakeven": "Breakeven (Target 1-2 banked, rest closed flat)",
    "trailing_stop": "Trailing stop hit (Target 1-3 banked)",
...
# این سه نماد همه‌ی TIMEFRAMES رو دارن (۱۵د،۱س،۴س،روزانه،هفتگی) - بدون محدودیت.
...
def format_results_message(trade_history: List[Dict[str, Any]]) -> Optional[str]:
    ...
    return (
        ...
        f"<i>{stats['total']} recently closed trades - every signal sent, no cherry-picking</i>\n\n"
...
def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any], ema7_now: Optional[float]) -> List[Dict[str, Any]]:
    ...
    (لحظه‌ای، بدون هزینه)، برای طلا از REST دوره‌ای (چون WebSocket رایگان ندارد).
    این یعنی هر معامله‌ی باز حداقل از دو مسیر مستقل رصد می‌شود؛ نتیجه‌ی هر بسته‌شدن
    (تارگت/استاپ/ریسک‌فری/تریلینگ/رانر/سیگنال مخالف) همیشه به‌صورت close_type دقیق
    ثبت و در پیام (خط Result) و در trade_history گزارش داده می‌شود.
    """
...
def check_open_trade_live(trade: Dict[str, Any], live_price: float) -> List[Dict[str, Any]]:
    ...
    رهگیری زنده‌ی معامله‌ی باز بین دو بسته‌شدن کندل (از WebSocket یا REST /price).
    منطق (تارگت/استاپ/ریسک‌فری/تریلینگ) دقیقاً همان check_open_trade است، فقط
    به‌جای high/low از یک نقطه‌ی قیمتی استفاده می‌کند."""
...
def send_trade_exit(bot: "TradeiscoolBot", symbol: str, display_name: str, tf_key: str, tf_label: str,
                     ...
                     source_label: Optional[str] = None, live_price: Optional[float] = None) -> int:
    """پیام خروج (تارگت/استاپ/ریسک‌فری/تریلینگ/رانر) رو با چارت می‌فرسته و در صورت
    بسته‌شدن معامله، نتیجه رو ثبت می‌کنه. هم از مسیر کندلی (process_and_send_symbol_tf)
    و هم از مسیر قیمت زنده (WebSocket/REST live) صدا زده می‌شه - یک نقطه‌ی مشترک،
    بدون تکرار منطق."""
    if event["type"] == "stop":
        title_suffix = "Stop"
        ...
        title_suffix = f"Target {event['level']}"
    chart = build_chart_from_hist(hist, f"{display_name} {tf_label} · {title_suffix}", trade=trade, live_price=live_price)
    if event["type"] == "stop":
...
TARGET_ACTION_LINE = {
    1: "Close 50% of the position here.",
    2: "Close 50% of the remainder and move stop to entry (risk-free).",
    3: "Close 70% of the remainder and start trailing the stop.",
    4: "Close 85% of the remainder. The final 15% (\"runner\") stays open on the "
       "same trailing stop in case the move extends beyond 4R.",
}
...
def format_entry_message(symbol: str, display_name: str, timeframe_label: str, sig: Dict[str, Any],
    ...
    targets_lines = "\n".join(
        f"🎯 {TARGET_LABELS[t]}: {format_signal_price(trade['entry'] + sign * t * trade['r'])}" for t in RR_TARGETS
    )
    body = (
...
def format_breakeven_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    return f"⚪ BREAKEVEN — {label} {tf_en}\n\n{body}\n\n{ts}\n\n{SIGNAL_FOOTER}"
...
def format_trailing_stop_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    return f"🔒 TRAILING STOP — {label} {tf_en}\n\n{body}\n\n{ts}\n\n{SIGNAL_FOOTER}"
...
def format_runner_stop_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    return f"🏁 RUNNER CLOSED — {label} {tf_en}\n\n{body}\n\n{ts}\n\n{SIGNAL_FOOTER}"
...
# ==================================================================
def run_live_gold_check_pass(bot: "TradeiscoolBot", state: Dict[str, Any]) -> None:
    """رهگیری REST قیمت زنده برای معاملات باز طلا (XAU) - چون هیچ WebSocket رایگانی
    برای طلا وجود ندارد، همیشه از همین مسیر رهگیری می‌شود. برای BTC/ETH این پاس REST
    عمداً اجرا نمی‌شود چون WebSocket پوشش لحظه‌ای و بدون هزینه‌ی API می‌دهد؛ اگر
    WebSocket قطع باشد، رهگیری معمولی مبتنی بر کندل (هر دور اسکن) همچنان کار می‌کند،
    فقط با دقت کندل به‌جای لحظه‌ای."""
    ...
    open_gold_combos = [
        key for key, s in candle_states.items()
        if key.startswith("XAU/USD|") and s.get("open_trade") and not s["open_trade"].get("closed")
    ]
    due = [k for k in open_gold_combos if (now_ts - live_last_checked.get(k, 0)) >= LIVE_CHECK_INTERVAL_SECONDS]
    due.sort(key=lambda k: live_last_checked.get(k, 0))
    due = due[:MAX_LIVE_CHECKS_PER_RUN]

    if due:
        logger.info(f"🔎 رهگیری REST قیمت زنده‌ی طلا برای {len(due)} معامله‌ی باز...")
    for state_key in due:
        symbol, tf_key = state_key.split("|")
        tf_cfg = TIMEFRAMES.get(tf_key)
        sym_state = candle_states.get(state_key)
        if not sym_state or not tf_cfg:
            continue
        trade = sym_state.get("open_trade")
        if not trade or trade.get("closed"):
            continue
        try:
            ...
            if live_price:
                events = check_open_trade_live(trade, live_price)
                for ev in events:
                    send_trade_exit(bot, symbol, "GOLD", tf_key, tf_cfg["label"], trade, ev,
                                     sym_state["hist"], trade_history=trade_history, live_price=live_price)
            live_last_checked[state_key] = now_ts
            time.sleep(7.5)  # رعایت محدودیت نرخ درخواست Twelve Data
        except Exception as e:
            logger.warning(f"⚠️ رهگیری زنده‌ی {state_key} ناموفق بود: {e}")


def run_scan_cycle(state: Dict[str, Any]) -> None:
    """یک دور کامل اسکن: سیگنال جدید در همه‌ی گروه‌ها (واچ‌لیست/۳۰-برتر/پرتحرک) +
    اسکن جریان استیبل‌کوین + گزارش روزانه (هر کدام فقط اگر due باشند) + رهگیری REST
    زنده‌ی طلا. این تابع هر SCAN_CYCLE_INTERVAL_SECONDS از حلقه‌ی اصلی صدا زده می‌شود."""
    ...
    run_live_gold_check_pass(bot, state)
