...
def _id_line(trade: Dict[str, Any]) -> str:
    """خط شناسه‌ی یکتای سیگنال («🆔 ALT-004821 #ALT_004821») - طبق درخواست صریح کاربر در همین
    گفتگو اضافه شد (برخلاف بقیه‌ی خطوط اضافه‌ای که قبلاً عمداً از این پیام‌ها حذف
    شدند - توضیح بالای format_entry_message)، چون این یک استثنای مشخص و هدفمند است:
    بدون این خط، وقتی این سیگنال به کانال VIP هم کپی می‌شود، هیچ راهی برای اشاره‌ی
    دقیق به همین یک سیگنال (نه یک سیگنال دیگر روی همان نماد) وجود ندارد - نه برای
    کاربر، نه برای ادمین. اگر شناسه موجود نباشد (رکورد خیلی قدیمی)، رشته‌ی خالی
    برمی‌گرداند - هیچ خط اضافه‌ای اضافه نمی‌شود.

    🔴 هماهنگ‌سازی با ربات مرجع: قسمت هشتگ («#ALT_004821») تازه اضافه شد - عیناً همون فرمتی
    که _id_line در candle_engine.py الان تولید می‌کنه. دو فایده‌ی مستقیم: (۱) با تپ‌کردن روی
    این هشتگ در اپ تلگرام، تمام پیام‌های همین یک سیگنال (ورود + هر تارگت/استاپ) در یک نتیجه‌ی
    جستجوی داخل کانال جمع می‌شن. (۲) subscription_bot.py (ربات مشترکینِ کانال VIP) دقیقاً
    همین خط 🆔 رو با یک regex می‌خونه تا وقتی این پیام به کانال VIP رله می‌شه، شناسه‌ی واقعیِ
    این معامله («ALT-004821») - نه یک شناسه‌ی تازه‌ساخته‌شده‌ی داخلی - رو به‌عنوان signal_id
    همون معامله در سیستم VIP هم ثبت کنه؛ بدون این خط، /admin_close_trade یا /signal روی سیستم
    VIP هیچ‌وقت این سیگنال رو پیدا نمی‌کردن."""
    sid = trade.get("signal_id")
    return f"🆔 {sid}  #{_id_hashtag(sid)}\n" if sid else ""
