...
#
# نکته‌ی مهم درباره‌ی «۱ دقیقه‌ای»: خودِ داده‌ی کندل ۱-دقیقه‌ای رایگان است (کریپتو
# در Twelve Data، بر خلاف کالاها، نیاز به پلن پولی ندارد)، ولی درخواست‌کردن آن به
# فاصله‌ی واقعیِ هر ۱ دقیقه برای حتی یک نماد به‌تنهایی ۱۴۴۰ درخواست/روز لازم دارد -
# یعنی به‌تنهایی از کل سهمیه‌ی روزانه (۸۰۰) بیشتر است. راه‌حل (مثل ۱۵دقیقه‌ای قبلی):
# هر دور اسکن (هر ۵ دقیقه) آخرین ۱۰ کندل ۱-دقیقه‌ای بسته‌شده خوانده می‌شود - پس هیچ
# کندلی از قلم نمی‌افتد و دقتِ سیگنال دقیقاً روی بسته‌شدن کندل ۱ دقیقه‌ای است، فقط
# کشفِ آن ممکن است تا ۵ دقیقه (نه بیشتر) از تشکیل واقعی‌اش عقب‌تر باشد - در عمل
# سریع‌ترین و دقیق‌ترین تایم‌فریمِ ممکن با سهمیه‌ی رایگان.
TIMEFRAMES = {
    ...
    "30m": {"td_interval": "30min", "bar_seconds": 30 * 60,          "label": "30M"},
...
def _tf_window_id(tf_key: str, now_utc: datetime) -> Optional[str]:
    ...
    هر تایم‌فریم دقیقاً به فاصله‌ی خودش (نه بیشتر، نه کمتر) پردازش می‌شود - هیچ
    کندلی هم از قلم نمی‌افتد چون هر بار آخرین ۱۰ کندل بسته‌شده خوانده می‌شود. برای
    «1m» و «5m» (که طول کندلشان از فاصله‌ی هر دور اسکن یعنی ۵ دقیقه کوتاه‌تر یا
    برابر است)، پنجره روی همان بازه‌ی ۵دقیقه‌ای دور اسکن قفل شده - یعنی این دو
    تایم‌فریم عملاً در هر دور اسکن (سریع‌ترین ممکن با سهمیه‌ی رایگان) بررسی می‌شوند.
    """
    if tf_key in ("1m", "5m"):
    ...
    if tf_key == "15m":
        ...
        return now_utc.strftime("%Y-%m-%d-%H-") + f"{bucket_min:02d}"
    if tf_key == "30m":
        bucket_min = (now_utc.minute // 30) * 30
        return now_utc.strftime("%Y-%m-%d-%H-") + f"{bucket_min:02d}"
    if tf_key == "1h":
...
# لیست «واچ‌لیست اصلی» - نمادهایی که همه‌ی TIMEFRAMES (۱د/۵د/۱۵د/۳۰د/۱ساعته) را
# بدون محدودیت دارند (کلید رایگان لازم است: https://twelvedata.com). طبق درخواست
# صریح کاربر: بیت‌کوین و اتریوم کامل از این ربات حذف شدند - این کانال فقط برای
...
#
# ۴ آلت‌کوین با بیشترین نقدشوندگی/حجم بازار انتخاب شدند (هم روی Coinbase برای
# استریم زنده، هم روی Twelve Data برای کندل، به‌خوبی پشتیبانی می‌شوند) - برای
# اضافه/حذف نماد همین دیکشنری (و WS_SYMBOL_MAP پایین‌تر، اگر نماد جدید نیاز به
# استریم زنده هم دارد) را ویرایش کنید. نمادهای «۳۰ ارز برتر» و «ارزهای پرتحرک»
# پایین‌تر به‌صورت خودکار و پویا از CoinMarketCap اضافه می‌شوند - نیازی به دستکاری
# دستی این دیکشنری برای آن‌ها نیست.
WATCHLIST_SYMBOLS = {
    "SOL/USD": "SOL",
    "XRP/USD": "XRP",
    "ADA/USD": "ADA",
...
# سرعت» برای آلت‌کوین‌ها) حالا دقیقاً همان ۴ آلت‌کوین واچ‌لیست اصلی را پوشش می‌دهد -
# یعنی معاملات باز این ۴ نماد هر ۵ ثانیه با آخرین تیک قیمت زنده (نه فقط هر چند
# دقیقه با کندل REST) چک می‌شوند؛ رصد تارگت/استاپ آن‌ها عملاً لحظه‌ای است.
# نمادهای «۳۰ ارز برتر» و «ارزهای پرتحرک» همچنان فقط از طریق کندل REST (در همان
# فاصله‌ی هر دور اسکن) رصد می‌شوند - چون تعدادشان پویا و بزرگ است، مشترک‌کردن آن‌ها
# در یک اتصال WebSocket واحد پیچیدگی/هزینه‌ی زیادی اضافه می‌کند بدون سود متناسب.
...
#
# طبق درخواست کاربر (فقط تایم‌فریم زیر ۴ ساعت + نهایت سرعت/دقت)، تایم‌فریم ۴ساعته‌ی
# این گروه حذف و به‌جایش «5m» اضافه شد - یعنی این گروه حالا سه تایم‌فریم دارد:
# ۱ساعته (لحظه‌ای، هر ساعت)، ۱۵دقیقه‌ای (هر ۲ ساعت، به‌خاطر سقف سهمیه) و ۵دقیقه‌ای
# (هر ۳ ساعت، هم‌زمان با خودِ بروزرسانی لیست کاندیدها - چون فرکانس درخواست ۵دقیقه‌ای
# روی ۱۲ نماد اگر بیشتر از این می‌شد به‌تنهایی از کل سهمیه‌ی روزانه عبور می‌کرد).
# چون هر بار آخرین ۱۰ کندل بسته‌شده خوانده می‌شود، هیچ کندلی از قلم نمی‌افتد - فقط
# کشفِ سیگنال ممکن است تا فاصله‌ی همان چک بعدی از تشکیل واقعی‌اش عقب‌تر باشد.
...
HOT_MOVERS_5M_SCAN_HOURS = [0, 3, 6, 9, 12, 15, 18, 21]                  # هر ۳ ساعت
...
def _td_unsupported_warning_line(symbol: str) -> Optional[str]:
    ...
    این سیگنال خاص را فوروارد نکند. نمادهای واچ‌لیست اصلی (SOL/XRP/ADA/DOGE) عملاً
...
def process_and_send_symbol_tf(bot: "TradeiscoolBot", fetch_fn, symbol: str, display_name: str,
    ...
    def _send_entry(sig, trade, hist):
        ...
        warn_line = _td_unsupported_warning_line(symbol)
        combined_extra = "\n".join(line for line in (extra_line, warn_line) if line) or None
        msg = format_entry_message(symbol, display_name, tf_label, sig, trade, bar_seconds=bar_seconds,
                                    source_label=source_label, extra_line=combined_extra)
...
TARGET_ACTION_LINE = {
    1: "✂️ Close 20% of your total position here, and move your stop-loss to Entry (risk-free from here).",
    ...
    4: (f"✂️ Close 10% of your total position here. The remaining 25% (\"runner\") is no longer "
        f"managed by a fixed stop - it now trails the price and closes automatically if it pulls "
        f"back {TRAILING_DISTANCE_R}R from the best level reached."),
}
...
def _result_line(trade: Dict[str, Any]) -> str:
    """طبق درخواست کاربر: عبارت‌بندی دقیقاً مطابق تصویر نمونه («📊 Result so far:
    +2.10R (profit)») - وقتی معامله هنوز کامل بسته نشده (مثلاً فقط یک تارگت خورده و
    رانر باز است) از «Result so far» استفاده می‌شود؛ وقتی کاملاً بسته شده از «Result»
    (بدون «so far»). کلمه‌ی profit/breakeven/loss همیشه از روی علامت واقعی final_r
    محاسبه می‌شود، نه از نوع بسته‌شدن، تا همیشه دقیق باشد."""
    result_r = compute_final_r(trade)
    word = "profit" if result_r > 0 else ("breakeven" if result_r == 0 else "loss")
    label = "Result" if trade.get("closed") else "Result so far"
    return f"📊 {label}: {result_r:+.2f}R ({word})"
...
def format_exit_message(symbol: str, display_name: str, timeframe_label: str, trade: Dict[str, Any],
    ...
    کاربر، هیچ سیگنالی بدون توضیح نحوه‌ی بسته‌شدنش نمی‌ماند: این پیام همیشه توضیح
    کامل می‌دهد که این بسته‌شدن دقیقاً چند درصد پوزیشن را شامل می‌شود و علت
    (close_reason) چیست - چه به تارگت رسیده باشد (format_rr_exit_message) چه استاپ
    خورده باشد (همین تابع)."""
...
def candle_signal_scan_job(bot: "TradeiscoolBot", state: Dict[str, Any], now_utc: datetime) -> List[Dict[str, Any]]:
    ...
    # تایم‌فریم، هر ۴ آلت‌کوین واچ‌لیست پشت‌سرهم و در کوتاه‌ترین فاصله‌ی ممکن از هم
    # چک می‌شوند - به‌جای اینکه مثلاً چک DOGE برای همان کندل به‌خاطر چک‌شدن چند
    # تایم‌فریم دیگر برای SOL قبل از آن، چند ده ثانیه عقب بیفتد.
    ...
    if budget_exceeded:
        ...
        if not TWELVEDATA_API_KEY:
            ...
            excluded = {"BTC", "ETH"}  # طبق درخواست کاربر این کانال کاملاً اختصاصی آلت‌کوین است
            top30_symbols = []
            for c in top_coins:
                sym = c.get("symbol", "")
    ...
    def get_hot_mover_symbols(self, count_each: int = 6) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            for coin in top_coins:
                try:
                    symbol = coin['symbol']
                    if symbol in STABLECOINS or symbol in ("BTC", "ETH"):
                        continue  # کانال کاملاً اختصاصی آلت‌کوین است - BTC خودِ معیار مقایسه است، ETH هم طبق درخواست حذف شد
