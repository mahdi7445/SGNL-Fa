...
# وزن هر مرحله از حجم اصلی پوزیشن (طبق درخواست کاربر، به‌روزرسانی‌شده)
#   تارگت ۱ (R=1): بستن ۳۵٪ حجم پوزیشن
#   تارگت ۲ (R=2): بستن ۳۵٪ از باقی‌مانده + انتقال حد ضرر به نقطه‌ی ورود (ریسک‌فری)
#   تارگت ۳ (R=3): بستن ۶۰٪ از باقی‌مانده + شروع تریل‌کردن حد ضرر
#   تارگت ۴ (R=4): بستن ۸۰٪ از باقی‌مانده + ۲۰٪ باقی («رانر») با همان تریلینگ‌استاپ باز می‌ماند
_T1_PCT, _T2_PCT_OF_REMAINDER, _T3_PCT_OF_REMAINDER = 0.35, 0.35, 0.60
_W1 = _T1_PCT
_W2 = (1 - _W1) * _T2_PCT_OF_REMAINDER
_W3 = (1 - _W1 - _W2) * _T3_PCT_OF_REMAINDER
_REMAINDER_AT_T4 = 1 - _W1 - _W2 - _W3
_RUNNER_KEEP_FRACTION = 0.20
...
# طبق درخواست کاربر: تمرکز اصلی روی آلت‌کوین‌ها در تایم‌فریم‌های پایین. بعد از حذف
# طلا از WATCHLIST_SYMBOLS، سهمیه‌ی آزادشده به این گروه (که تنها گروه آلت‌کوین با
# تایم‌فریم پایین است) اختصاص یافت: هم تعداد ارزها بیشتر شد (۶→۸ برنده/بازنده) و
# هم تایم‌فریم ۱ساعته (بین ۱۵د و ۴س) اضافه شد.
HOT_MOVERS_COUNT_EACH = 8   # ۸ برنده + ۸ بازنده = ۱۶ ارز
...
def _hot_movers_window_id(tf_key: str, now_utc: datetime) -> str:
    ...
    # 15m و 1h برای ارزهای پرتحرک: هر ۲ ساعت (به‌خاطر محدودیت سهمیه - توضیح در تعریف
...
def send_trade_exit(bot: "TradeiscoolBot", symbol: str, display_name: str, tf_key: str, tf_label: str,
    ...
    """پیام خروج (تارگت/استاپ/ریسک‌فری/تریلینگ/رانر) رو می‌فرسته و در صورت
    ...
    طبق درخواست کاربر، پیام‌های خروج (استاپ/تارگت/ریسک‌فری/تریلینگ/رانر) دیگر
    عکس چارت ندارند - فقط متن، برای مختصر و سریع بودن. عکس چارت فقط در پیام
    سیگنال ورود اولیه (process_and_send_symbol_tf._send_entry) فرستاده می‌شود."""
    if event["type"] == "stop":
...
TARGET_ACTION_LINE = {
    1: "Close 35% of the position here.",
    2: "Close 35% of the remainder and move stop to entry (risk-free).",
    3: "Close 60% of the remainder and start trailing the stop.",
    4: "Close 80% of the remainder. The final 20% (\"runner\") stays open on the "
