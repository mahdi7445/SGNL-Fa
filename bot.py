...
def format_candle_signal_message(symbol: str, sig: Dict[str, Any], display_name: Optional[str] = None,
                                  timeframe_label: str = "۴ ساعته", bar_seconds: int = 0) -> str:
    side = sig["side"]
    ...
    return f"""{emoji} <b>{title}</b> | {clean_symbol} ({symbol}) — کندلی {timeframe_label}

قیمت: {format_price(price)}
تایید: {confirm_txt}
زمان تشکیل: {ts}

⚠️ سیگنال خام - مدیریت ریسک فراموش نشود"""
    ...
    def format_flow_signal_message(self, signal_data: Dict[str, Any]) -> Optional[str]:
        try:
            if signal_data['side'] == "LONG":
                ...
                emoji, title = "🔴", "SHORT"
            return f"""{emoji} <b>{title}</b> | {signal_data['name']} ({signal_data['symbol']}) — جریان استیبل‌کوین

امتیاز: {signal_data['total_score']}/12
قیمت: {format_price(signal_data['price'])} | مارکت‌کپ: {format_number(signal_data['market_cap'])}
خالص جریان: {format_number(signal_data['net_flow'])} (ورودی {format_number(signal_data['inflow'])} / خروجی {format_number(signal_data['outflow'])})
عملکرد نسبی به BTC: {format_percent(signal_data['relative_performance'])}
    ...
    def format_performance_signal_message(self, signal_data: Dict[str, Any]) -> Optional[str]:
        try:
            if signal_data.get('side') == "LONG":
                ...
                emoji, title = "🔴", "SHORT"
            return f"""{emoji} <b>{title}</b> | {signal_data.get('name')} ({signal_data.get('symbol')}) — عملکرد نسبی

امتیاز: {signal_data.get('total_score', 0)}/12 | رتبه بازار: #{signal_data.get('rank', 0)}
قیمت: {format_price(signal_data.get('price', 0))} | مارکت‌کپ: {format_number(signal_data.get('market_cap', 0))}
تغییر 24h: {format_percent(signal_data.get('change_24h', 0))} | نسبت به BTC: {format_percent(signal_data.get('relative_performance', 0))}
