    ...
    def on_message(ws, message):
        ...
        if parsed:
            ...
            if parsed["symbol"] in TREND_ALTS_WS_SYMBOL_MAP:
                feed_1m_candle_tick(parsed["symbol"], parsed["price"], time.time())
...
def check_ws_open_trades(bot: "TradeiscoolBot", state: Dict[str, Any],
    ...
    for mover in state.get("trend_alts", {}).get("symbols", []):
...
def check_trend_alts_1m_signals(bot: "TradeiscoolBot", state: Dict[str, Any]) -> None:
    ...
    trend_alts = state.get("trend_alts", {}).get("symbols", [])
    if not trend_alts:
        return
    candle_states = state.setdefault("candle_signals", {})
    trade_history = state.setdefault("trade_history", [])
    ...
    for mover in trend_alts:
        ...
        try:
            new_state, sent, err = process_and_send_symbol_tf(
                ...
                extra_line=perf_line, target_events=target_events)
...
def build_trend_alts_ws_map(state: Dict[str, Any]) -> Dict[str, str]:
    """نگاشت «TICKER/USD → TICKER-USD» برای ارزهای همسو با روندِ همین روز - برای
...
def feed_1m_candle_tick(symbol: str, price: float, ts: float) -> None:
    ...
    with _candle_builder_lock:
        ...
        if cur is None or cur["bucket"] != bucket_start:
            ...
            b["cur"] = {"bucket": bucket_start, "o": price, "h": price, "l": price, "c": price}
        else:
            if price > cur["h"]:
                cur["h"] = price
            if price < cur["l"]:
...
def candle_signal_scan_job(bot: "TradeiscoolBot", state: Dict[str, Any], now_utc: datetime) -> List[Dict[str, Any]]:
    ...
    trend_alts_info = state.get("trend_alts", {})
    trend_alts = trend_alts_info.get("symbols", [])
    regime = trend_alts_info.get("regime")
            ...
            for mover in trend_alts:
                ...
                try:
                    ...
                    new_state, sent, err = process_and_send_symbol_tf(
                        ...
                        trade_history=trade_history, source_label=f"ارز همسو با روند ({mover['reason']})", extra_line=perf_line,
                        target_events=target_events)
                    if new_state is None:
                        entry["error"] = err or "دریافت داده از Twelve Data ناموفق بود (ممکن است این ارز پشتیبانی نشود)"
                    else:
...
class TradeiscoolBot:
    ...
    # ---------------- ارزهای همسو با روند بازار (نسبت به BTC، انتخاب روزانه) ----------------
    def get_trend_alts(self, count: int = TREND_ALTS_COUNT) -> Optional[Dict[str, Any]]:
        """
        ...
        سیگنال واقعی را طبق قبل موتور کندلی (پین‌بار/اینگولفینگ + EMA7/EMA25 + فیلتر
        ADX) صادر می‌کند - این تابع فقط کاندیدها را انتخاب می‌کند.
        """
        try:
    ...
    def refresh_trend_alts(self, state: Dict[str, Any]) -> None:
        result = self.get_trend_alts(TREND_ALTS_COUNT)
        if result is None:
        ...
        if result['regime'] == 'neutral':
            ...
            logger.info(f"📈 بازار {direction_fa} است (BTC 24h: {format_percent(result['btc_change'])}) - "
                        f"لیست امروز: {', '.join(c['symbol'] for c in result['symbols'])}")
