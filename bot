        ...
        if has_local_changes:
            ...
            if push_ok:
                ...
                logger.error("❌ [git] پوش بعد از ۳ تلاش هم ناموفق بود - علامت‌های state.json "
                              ...
                              "این حلقه (یا اجرای بعدی) دوباره تلاش می‌کند")
...
def _refresh_results_report_flag_from_disk(state: Dict[str, Any]) -> None:
    """این تابع پرچم «گزارش روزانه امروز فرستاده شد یا نه» (tf_windows.results_report)
    را از فایل تازه‌ی روی دیسک در state موجود merge می‌کند - نه کل state را، چون همین
    دور اسکن ممکن است خودش تغییرات پردازش‌نشده/پوش‌نشده‌ی دیگری (مثل candle_signals
    تازه) داشته باشد که نباید با نسخه‌ی قدیمی‌تر دیسک بازنویسی شوند.

    🔴 رفعِ باگِ واقعیِ «گزارش روزانه ده‌ها بار در روز تکرار می‌شود»: نسخه‌ی قبلی این
    تابع بدون قید‌وشرط هر چیزی که روی دیسک بود را جایگزین حافظه می‌کرد (`if disk_flag:
    state[...] = disk_flag`) - even اگر آن مقدار از چیزی که همین الان در حافظه است
    قدیمی‌تر باشد! ترکیب این با یک باگ دیگر (رفع‌شده در تغییر قبلی: state_backup
    revert در حلقه‌ی اصلی که هر اجرای run_scan_cycل را با خطا کامل به نسخه‌ی قبل از
    شروع آن دور برمی‌گرداند) این چرخه‌ی معیوب را می‌ساخت: ۱) گزارش فرستاده و پرچم
    امروز در حافظه ست می‌شود، ۲) اگر همان دور اسکن (حتی در save_td_unsupported_cache/
    sync_state انتهای تابع) با خطا مواجه شود، کل state (از جمله همین پرچم) به نسخه‌ی
    قبل از این دور برمی‌گشت، ۳) دور بعدی (۵ دقیقه بعد)، این تابع دوباره از دیسک
    می‌خواند - که چون آخرین save موفق قبل از خطا هنوز پرچم امروز را نداشت، دوباره
    مقدار قدیمی را در حافظه می‌نشاند، ۴) is_results_report_due دوباره True می‌شود و
    گزارش دوباره فرستاده می‌شود - این چرخه می‌توانست ده‌ها بار در طول روز تکرار شود
    (هر بار آن خطای خاص دوباره رخ می‌داد). حالا دو لایه‌ی مستقل این را می‌بندند:
    (الف) اینجا: دیسک فقط وقتی جایگزین حافظه می‌شود که مقدارش «جدیدتر یا مساوی» باشد
    (هرگز عقب‌گرد نمی‌کند)، (ب) در run_scan_cycle: مراحل انتهایی بعد از ارسال گزارش
    حالا در try/except جدا پیچیده شده‌اند تا خطای آن‌ها دیگر کل state (و این پرچم) را
    به عقب برنگرداند."""
    try:
        disk_state = load_state()
        disk_flag = disk_state.get("tf_windows", {}).get("results_report")
        mem_flag = state.get("tf_windows", {}).get("results_report")
        if disk_flag and (not mem_flag or disk_flag >= mem_flag):
...
def run_scan_cycle(state: Dict[str, Any]) -> None:
    ...
    bot.sync_state()
    # 🔴 لایه‌ی دوم دفاعی (لایه‌ی اول: عدم‌عقب‌گردِ _refresh_results_report_flag_from_disk
    # بالاتر) در برابر همان چرخه‌ی معیوب «گزارش روزانه ده‌ها بار تکرار می‌شود»: این
